...
- **有效路径**：scanner 仅把完整 raw OHLCV 序列压缩存入 monitor 私有 SQLite checkpoint。新进程严格验证每根的周期对齐、连续性、有限 OHLCV 和价格边界；任何坏 payload 直接丢弃并冷取。恢复后仍调用 `OKX.candles(previous)`，只由交易所增量合并。
