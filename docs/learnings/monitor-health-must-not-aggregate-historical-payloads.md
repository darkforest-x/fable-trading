...
- **有效路径**：单独读取 `scan` 元数据、校准时钟和模型 gate 状态计算 health；状态页的市场计数改为 SQL 聚合，通知计数从 outbox 出发，避免无待发回执时扫描事件。完整市场/图表仍由专用 API 按需读取。YOLO 运行时也只在已有待确认原始信号时加载，避免空闲服务把模型预热和 liveness 混在同一启动路径。
