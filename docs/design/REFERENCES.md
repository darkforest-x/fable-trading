...
## Hummingbot Dashboard 结构摘要（源码）

```
Main
  └ landing.py          # hero + 4 metric cards + quick links
Config Generator
  └ bollinger_v1/...    # user_inputs → plotly candles+BB+markers → backtest metrics → save
Bot Orchestration
  └ instances / deploy / credentials / portfolio / trading
Data
  └ download_candles
```
...
- `st.navigation` 分组多页；`layout="wide"`；sidebar expanded  
- `.streamlit/config.toml`：`base="dark"`, `font="monospace"`  
- `visualization/theme.py`：plotly_dark、透明底、`hovermode=x unified`  
- `visualization/signals.py`：buy=triangle-up 蓝 / sell=triangle-down 红  
- 策略页固定节奏：inputs → `st.plotly_chart` → metrics columns → `render_save_config`

## 决策

| 方向 | 状态 |
|------|------|
| FreqUI 三栏 chart-first | mock：`terminal-mock.html` |
| **Hummingbot 侧栏多页 + 指标卡 + 表单→图** | mock：`hummingbot-mock.html`（用户指定试这个） |
| 实现层 | **不**整站迁 Streamlit；用现有 FastAPI+静态 HTML **仿壳与节奏**，图继续 LWC |
| 标注 | HB 式 markers（▲ 密集/信号），少全图矩形标签 |
