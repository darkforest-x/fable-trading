...
建立共用基础能力上的个人交易与量化研究体系。当前主要研究假设是：
...
业务分为三条并行线：**个人交易系统、策略研究、自动策略验证**。个人交易是长期业务能力，
不依赖自动策略先通过；人工判断本身也需要前向记录与复盘，不能继承模型或指标的回测收益。
...
统一前端 `#manual` 与 `/api/research/manual` 提供个人规则、盘前计划、机会与交易、复盘。
它消费同一信号中心和研究知识，由 Owner 看图、判断并在交易所执行；后台仅记录本人输入，
不连接账户、不下单，也不将手工练习混入自动模拟实盘。

- 个人规则版本化，均线启动和 BB 反转提供草稿。历史手册里的周期、风险比例和人工退出均需本人选择，
  不作为已经批准的默认值。`ready` 仅表示个人规则字段完整，由本人启用，不是研究盈利或生产资格。
- 盘前写市场背景、观察标的、重点、不做条件、值守时间与自定预算；机会卡关联盘前计划和规则版本。
- 观察与正式计划可以编辑；登记已成交后冻结原计划及引用快照，只追加持仓管理、平仓、收益核对与复盘。
  事后补录单独标记，不能冒充事前记录。记录超预算的实际成交，不阻止保留偏离事实。
- 首版仅记录 USDT 线性永续，数量为基础币数量。初始价格风险 = 入场到原始止损的距离 × 数量，
  不含费用、资金费或跳空。净收益由本人根据实际账单填写；缺失保持未知，初始 SL 缺失不算 R。
  加减仓或部分成交暂不自动核算，以最终归集记录及操作备注留痕。
- 真实交易与手工练习分别统计。复盘区分遵守规则和盈亏结果，失败与放弃机会也保留；
  形成的假设可转入既有实验流程，人工复盘本身不自动授予模型训练或策略生产资格。
