...
|------------|--------------------|----------------------|----------|----------|
| `build_dataset` | `python3 -m src.judgment.build_dataset` | `mode∈{strict,expanded}`；`bar∈BAR_CHOICES`；`horizon_bars∈[12,576]`；`out` 仅允许 MA206 预置模板名 | 2h | `data/ma206/judgment_dataset_*.csv` |
...
| `swap_replication` | `python3 scripts/swap_replication.py` | 无参或固定 flag 子集（若脚本后续加 flag，先改白名单再暴露） | 2h | `analysis/output/swap_replication_ma206.json` |
| `update_okx` | `python3 -m src.data.update_okx` | `bar∈BAR_CHOICES` | 1h | `data/kline_fetched/` |
| `forward_track` | `python3 scripts/forward_track.py` | 正式窗口固定，不向 UI 暴露 `start`；高级参数需 owner 批准后再开 | 30m | `data/forward_log_ma206.csv` |
...
- 前向日志健康：`data/forward_log_ma206.csv` 行数、最近 `detected_at`、距「100 笔裁决」进度（复用 forward payload 逻辑）。
...
Body: { "artifact_id": "frozen_tp5_sl2_swap_ma206_20260710" }
```
