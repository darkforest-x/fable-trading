...
## 当前周期（2026-09-09 22:00，v1.12.0）

六周期5m/15m/30m/1H/4H/日线均在前端展示；只有1H/4H/日线发送Bark原始启动和YOLO追加。
本次已同步客户端5m校验与interval=5，重启显示客户端；Windows网关运行规则、文件SHA及保存布局URL
已核对。下方初装五周期的实图验收记录保留；本次未重新打开WindowsTV做5m实图验收。
