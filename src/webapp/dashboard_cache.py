...
from src.backtest.run import build_signals, simulate, validation_start as model_validation_start
...
def universe_spec(universe: str = DEFAULT_UNIVERSE) -> UniverseSpec:
    ...
    if key not in UNIVERSES:
        raise HTTPException(400, f"unknown universe {universe!r}; MA206 mainline supports swap only")
    ...
    if spec.key not in _signals:
        ...
        if rebuild:
            ...
            meta_path.write_text(
                json.dumps(
                    _score_cache_metadata(spec, threshold, _artifact_for_spec(spec)),
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
...
def validation_start(universe: str = DEFAULT_UNIVERSE) -> pd.Timestamp:
    spec = universe_spec(universe)
    return model_validation_start(spec.dataset_path)


def symbol_matches_universe(symbol: str, universe: str) -> bool:
    return universe == "swap" and symbol.endswith("_USDT_SWAP")
...
def _cache_matches_universe(meta: dict, spec: UniverseSpec, artifact) -> bool:
    ...
    if meta.get("dataset_sha256") != file_sha256(spec.dataset_path):
        return False
    if meta.get("score_scope") != "pre_holdout_only":
        return False
    if meta.get("score_end_before") != str(HOLDOUT_START):
        return False
