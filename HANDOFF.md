...
5. 前向跟踪脚本已完成：`scripts/forward_track.py` 默认从
   ...
   当前正式窗口 0 条新信号；临时 `--start 2026-07-01` 回放 19 条，重复运行新增 0。
6. 前向验证窗口从 2026-07-08 起积累（每日 8 点自动更新数据），~3-4 周后用冻结的
   TP5/SL2+maker 配置做最终 PF 裁决。**加入每日定时任务前必须停下等 owner 点头。**
