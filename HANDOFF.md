...
## 最新：RSI第五个同色入场新假设，计数与退出待Owner确认（2026-09-20）

- Owner改为OKX BTC永续1h/4h：第五个同色时入场、不止损、反向大菱形时“慢慢止盈”。尚待明确第五个是SAR可见圆点/含翻转根的状态K线/强菱形，以及平仓比例、亏损时是否同样退出。不可替Owner选一个版本后宣称已完成回测。
- 信号准备`yoyo/evaluation/btc_rsi_fifth.py`保留state_run与visible_dot_run，前者第5根比后者早1根；UTC4h只用48根完整5m。79项信号/原SAR/层间检查通过，源码提交cb13985eb6。未运行新策略收益、未复用旧3R或R统计。
- 实验`experiments/active/exp-btc-rsi-fifth-color-20260920-v1/PROJECT_PLAN.md`已建档；复用上轮3年冻结OKX数据、20bp成本，Notion3e08856479af81169602db2278ae57d7标计划。Owner答复后固定执行规则，先提交builder再评分；持仓浮亏与末端未平仓必须报告。无Pine/训练/生产变更。
