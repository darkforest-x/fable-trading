...
- **现役检测器:无**。唯一在途:`owner_v16_tipuni_cold`(yolo11n 冷启动,3060 在训,
  统一管线数据集 v16;val 正样本已修为 tip 对齐——owner 目检抓出 v14 val 未对齐)。
  过金标验收(应开火 ≫ 记录基线 且 空背景误火 ≈0 + tip-smoke)并 owner 点头后才上。
