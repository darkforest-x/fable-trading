...
- 手机直接看`page_01.png`，对应R01/R02。等待Owner分别判断有/无/不确定、方向、核心首末编号、是否已启动。当前`answers.json`全空，人工标签0；不得把图已生成当成盲审已通过，不得自动给所有图打金标。Owner答复后只登记实际确认的编号，保留不确定项，另行对账冻结模型输出。
