...
## 最新：Owner已批准V4两模型离线训练例外，准备季度验证（2026-09-21）

- Owner在“是否批准仅这批数据的离线训练例外”后回复“继续”。唯一授权为`exp-spike-10r-model-20260921-v4/AUTHORIZATION.json`固定数据SHA、逻辑回归及小型LightGBM；不改全局P0/P1、YOLO、父产物资格或生产准入。
- 20因果特征加周期，原R/退出/20bp成本不变；每目标季之前12个月拆成9月训练与3月无标签分数门槛，跨训练切点未成熟标签剔除；2025Q4起四季前推。固定8模型门槛，全部失败结果也报告。
- 计划与源码先提交再训练，目标仍是从全原信号提高最终净R>10的命中率；未证明成功前不得称高胜率策略。V3/V3.1负面结论仍保留。
