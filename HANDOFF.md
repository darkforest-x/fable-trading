...
**2b 验收通过（2026-07-08，holdout 已消耗）→ 阶段 3 第一轮回测已跑：验收未通过**——
验收窗口 PF 1.01 < 1.3（基准成本 0.3%；0.2% 时 1.30 擦线），病根是单笔 edge（~0.10%）
对成本无安全边际。详见 `analysis/p3_backtest_report.md`。
**等待 owner 在报告第 6 节的 A/B/C/D 选项中拍板**（推荐 D：maker 成本模型 + 前向数据积累）。
纪律红线：验收窗口已被看过两次，禁止再做任何参数级调优；只允许结构性改动。
