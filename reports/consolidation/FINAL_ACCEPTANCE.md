...
| 无新增测试失败 | PASS — 701 → **1181 passing / 0 failed**（exit 0），新增失败 **0** |
...
- ~~**11 个先存测试失败**~~ **已解决（2026-08-20）**：它们缺的是 gitignore 掉的 `data/` 产物。
  改为**条件跳过**并在原因里点名缺哪个文件——一个跑不了的测试不等于一个失败的测试，
  而 11 条常红会教会所有人无视 CI 徽章。本机有数据时它们照常运行（实测 55 passed）。
