...
原版 V1 的 ETH 样本过小，不能命名“最优止损”。30 分钟有 11 笔，表现主要由一笔 +19.83R 交易贡献；1 小时只有 3 笔且全亏，4 小时只有 1 笔。本轮**没有充分证据**改动默认 2 ATR 初始止损和 4 ATR 跟随止损。初始 ATR floor 的 1.5–3.0 平坦，是结构极值止损在这些事件中仍更宽、使 floor 从未成为约束，而不是优化成功；扩大到 4–6 ATR 后才确实改变了四笔以上的初始 SL，开发段都更差。3 ATR 跟随在开发和后段都降低 30 分钟净 R；5 ATR 没有稳定改善。

Freqtrade 2026.8 已实际运行三周期基线回测和两个 30 分钟、单线程、小预算 Hyperopt。它用预先由原始 V1 replay 计算的“上一根收盘已知”止损桥接，因此不会犯“本根收盘上移止损、又用本根 low 触发”的前视错误。三个周期合计 15 笔的开/平仓价与冻结账本逐笔一致（30m=11、1h=3、4h=1）；Freqtrade 的 stop 平仓时间标签是该根开盘，研究账本记录该根结束，价格相同。
...
| 初始 3.0 ATR | 5 / +8.740 / 9.024 | 6 / +23.474 / 8.469 | 仍由结构 SL 控制，逐笔相同 |
| 初始 4.0 ATR | 5 / +20.362 / 19.694 | 6 / +22.902 / 8.288 | 改变初始 SL；开发提高、后段略低，样本不足 |
| 初始 5.0 ATR | 5 / +17.549 / 17.336 | 6 / +18.687 / 6.956 | 两段均非稳定优势 |
| 初始 6.0 ATR | 5 / +14.776 / 14.915 | 6 / +17.821 / 9.546 | 两段均低于默认 |
| trail 3 ATR | 5 / +7.028 / 7.452 | 6 / +21.711 / 7.908 | 两段均较差 |
| trail 5 ATR | 5 / +8.318 / 6.829 | 6 / +22.100 / 8.032 | 无稳定改善 |

全三周期合计时，开发段 7 笔的默认净 R 为 +6.654，后段 8 笔为 +24.004；这个合计仍然被 30m 主导，不能掩盖 1h 的负结果。初始止损触发比例为 30m 开发 20%、后段 50%。被初始 stop 的交易在其后的 20 根里，**从 entry 计算的未来最高价收益**平均为 -2.10% 和 -3.30%；它可以为负，故不是标准的持仓期 MFE，只是“止损后是否快速重回 entry 上方”的标签，不能用来证明未被洗掉。跟随 stop 触发比例为 30m 开发 80%、后段 50%。

表中每个 30m 格均是 realized 事件；所有胜率、PF 和净 R 均排除 censored。trail 5 ATR 的后段在 30m 是 6 realized / 0 censored；若合并三周期，则另有 1 个 4h `boundary_mark` censored 事件，未计入任何收益、胜率或 PF。Freqtrade 的窗口末尾 force-exit 也不当作策略真实 exit。
...
- 项目定义的 holdout 从 2026-05-04 起；本轮在 owner 已授权下读取了该段冻结数据，因此发生了 holdout 暴露。历史消耗次数没有可用 receipt，不能猜测次数；开发/后段都属于已被研究过的冻结两年历史，不能改称首次未见样本外。
