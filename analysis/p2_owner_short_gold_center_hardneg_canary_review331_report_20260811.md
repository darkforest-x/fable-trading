...
- 审核页V3只保留“对 / 框偏 / 不对”三项主裁决，快捷键为`1 / 2 / 3`，分类后自动跳到下一张，
  `Z`撤销。默认331张全部未确认，只有Owner导出的裁决才能改变语义；本轮没有自动预选任何一张。
- V2未来对照图误用了YOLO训练渲染器的6%最小纵轴跨度：291/331张（87.92%）真实图幅不足6%，
  实际波幅中位数仅2.99%，因此被视觉压平。V3为审核图单独使用真实高低点+均线自动缩放，取消
  训练下限，并在图头和卡片元数据中显示每张实际波幅。
...
自动等价为真目标。右图按本图OHLC和六条均线的真实极值独立自动缩放，图头`AUTO-Y`显示未加边距前
的真实跨度。训练输入仍只允许使用左侧对应的原始因果文件；审核页中带橙框的左图也是单独副本，
避免框线像素污染训练输入。
...
| 完整48根未来 | 328 |
| 仅47根未来 | 3 |
| 未来真实波幅p10 / median / p90 | 1.50% / 2.99% / 6.75% |
| 未来审核纵轴 | 真实OHLC+MA极值自动缩放，无6%训练下限 |
...
- 实测连续按`1 / 2 / 3`依次把C001/C002/C003标成对/框偏/不对，并自动移动当前卡；按`Z`
  后C003恢复未确认。JSON导出包含V3 protocol、源事件SHA、331个完整decision和正确计数。
- 验收后已清除浏览器测试选择并刷新，交付页恢复`pending=331`。
- V2→V3的331张原始因果图和331张带框因果副本SHA逐行一致；仅331张未来审核图发生变化。
- 全量测试：`641 passed, 2 skipped`。
...
- V3每张未来图独立自动缩放，因此不能用不同卡片中K线的视觉高度直接比较绝对波动；应读取卡片和
  图头显示的真实波幅百分比。该缩放只服务人工辨形态，不进入模型输入。
...
- 冻结清单：
  `analysis/output/owner_short_gold_center_hardneg_canary_review331_v3/review_manifest.jsonl`
- 机器审计摘要：
  `analysis/output/owner_short_gold_center_hardneg_canary_review331_v3/summary.json`
