...
- 审核页V2只保留“对 / 框偏 / 不对”三项主裁决，快捷键为`1 / 2 / 3`，分类后自动跳到下一张，
  `Z`撤销。默认331张全部未确认，只有Owner导出的裁决才能改变语义；本轮没有自动预选任何一张。
...
| 按钮 | 含义 | 是否可直接写成训练负例 |
|---|---|---|
| 对（快捷键1） | 形态和核心位置都正确 | 否；应保留为正例候选 |
| 框偏（快捷键2） | 形态正确，但模型核心框边界有偏差 | 否；需要Owner重框或回到原金标几何 |
| 不对（快捷键3） | 当时可见形态就不是目标 | 仅为hard-negative候选；仍需数据去重和时间切分 |

页面右侧未来最多48根只帮助人工判断“后续是否真的启动/是否只是震荡”，不能反过来把“后来跌了”
自动等价为真目标。训练输入只允许使用左侧对应的原始因果文件；审核页中带橙框的左图也是单独副本，
避免框线像素污染训练输入。

“延续/重复”已从Owner主按钮删除：它属于相邻事件和去重规则诊断，不应与单张图片的形态/框裁决
混在同一操作层。后续由事件时间和核心重叠关系先自动聚类，再对必要的歧义组单独复核。
...
- 实测连续按`1 / 2 / 3`依次把C001/C002/C003标成对/框偏/不对，并自动移动当前卡；按`Z`
  后C003恢复未确认。JSON导出包含V2 protocol、源事件SHA、331个完整decision和正确计数。
- 验收后已清除浏览器测试选择并刷新，交付页恢复`pending=331`。
- V1/V2的331行三类图片SHA逐行一致，只改变审核交互，没有改变任何样本像素。
- 全量测试：`640 passed, 2 skipped`。
...
- 冻结清单：
  `analysis/output/owner_short_gold_center_hardneg_canary_review331_v2/review_manifest.jsonl`
- 机器审计摘要：
  `analysis/output/owner_short_gold_center_hardneg_canary_review331_v2/summary.json`

Owner先用`1 / 2 / 3`逐张完成三项裁决并复制页面底部JSON。收到裁决后，先统计真目标precision、
框偏率和明确负例率；延续重复率由相邻事件关系另行计算，再据此设计跨多个未使用pre-holdout时间块
的补挖策略。第三训练臂的正式数据构建和开训是下一次单独动作，不能由本审核页自动触发。
