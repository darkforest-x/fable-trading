...
20 个独立事件，扣 0.2% 往返成本后平均 +1.231%；
混合模型保留 41 个，净均值仅
+0.034%。但合并置换检验
`p=0.072093`，未达到 0.01；SHORT 仅保留
7 个，合计也低于预注册的30个独立事件门。
这只能作为“应继续积累同合同新样本”的发现，不能 promote、部署或用于下单。

## 冻结实验合同

- 来源数据 SHA-256：`e7b6f94cf59493c1f33e46c2f08fe5252a0a6e80406ab43a061fd112fc70f798`；候选、特征、TP5/SL2/72 标签与
  0.2% 成本均未重算或修改。
...
|---|---:|---:|---:|---:|---:|
| LONG | 1801 | 705 | 59 | 414 | 623 |
| SHORT | 1978 | 1074 | 99 | 407 | 398 |
| 合计 | 3779 | 1779 | 158 | 821 | 1021 |

## 时间外结果

| 模型 | 独立 train/tune/final | best iter | q90保留n | q90净收益 | q90胜率 | top-decile净收益 | 置换p | AUC |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 混合回归（复现基线） | 417 / 229 / 242 | 2 | 41 | +0.034% | 29.27% | -0.150% | 0.6564 | 0.5646 |
| LONG 独立回归 | 190 / 118 / 157 | 17 | 13 | +1.261% | 30.77% | +0.887% | 0.0966 | 0.5354 |
| SHORT 独立回归 | 227 / 111 / 85 | 17 | 7 | +1.175% | 57.14% | +1.086% | 0.1186 | 0.4350 |
| 多空独立后合并 | — / — / 242 | — | 20 | +1.231% | 40.00% | +0.939% | 0.0721 | 0.4972 |

LONG 的最终 top-decile 比同方向 `ma_spread_pct` 单特征基线高 +0.548%；SHORT 高
+1.556%。这说明改善不只是把数据按方向切开后的单特征分桶，但样本量仍不足以证明
稳定性。AUC 只作诊断：收益回归按幅度排序，不能用 TP-first 分类 AUC 作为成功裁决。

## 匹配随机对照

合并后的20个 q90 事件在8个固定分配上全部跑赢同币 × 同月 × 同UTC时段 × 同ATR桶 × 同方向
的随机候选；平均净超额 +1.134%，每个分配有
20 对。LONG 与 SHORT 单独也均为8/8正，但SHORT每个分配
...
| 门 | 结果 |
|---|---:|
| `mixed_reproduction` | PASS |
| `aggregate_frozen_threshold_net_strictly_better_than_mixed` | PASS |
| `aggregate_top_decile_net_positive` | PASS |
| `aggregate_outcome_permutation_p_lt_0_01` | FAIL |
| `aggregate_minimum_30_selected_dependency_blocks` | FAIL |
| `aggregate_beats_matched_controls_every_assignment` | PASS |
| `each_side_minimum_10_selected_dependency_blocks` | FAIL |
| `neither_side_frozen_threshold_net_negative` | PASS |
...
- 校验回执：`verify.passed=true`，全部
  12 项为真。

## 复现命令

```bash
git checkout 976d1b3f0edba6a8c44f8ee2ce2141ccc1a70ac0
