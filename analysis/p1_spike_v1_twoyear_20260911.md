...
| 周期 | 信号行 | 已实现 / censored | 已实现正收益率 | 已实现等权净收益和 | 已实现 PF | 事件序列回撤 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 30m | 2,271 | 2,260 / 11 | 29.60% | +0.8837 | 1.0093 | -12.7666 |
| 1H | 1,309 | 1,299 / 10 | 26.94% | +11.3079 | 1.1439 | -23.3025 |
| 4H | 400 | 369 / 31 | 29.00% | -12.1982 | 0.7166 | -21.3297 |
| 合计 | 3,980 | 3,928 / 52 | — | — | — | — |

这只是当前可获得单元的等权描述，正负值均不能视作策略的成功或失败结论。按入场年份、交易所和周期拆分的完整行在 CSV 中保留，避免将不同时间段混成一个成绩单。

**大趋势与失败原因。** 当前冻结 runner 的所有已实现行都是 `protective_stop` 退出，因此账本没有一个可用于宣称“成功大趋势”的已实现退出类别。该类 3,928 行的收益和为 -0.0065，中位 MFE 为 +5.81%，中位 MAE 为 -4.00%。这些是已覆盖样本中的路径描述，不能归因于单一市场机制，也不能推出应改变 V1 的风险线或阈值。
...
监控台中的 1,019 条 `replay/raw` 仍保持 `performance_status=unverified`。只有逐 event id 与独立经济审计 receipt 完整匹配，才可以把经过审计的账本字段挂到某一回放卡；本报告没有执行该关联。
