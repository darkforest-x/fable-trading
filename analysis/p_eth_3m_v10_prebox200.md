...
日期：2026-07-29  
状态：HTML 已生成；200 条“是/不是”任务已导入 Label Studio project 53 并完成界面与图片验收；未耗 holdout。
...
- Label Studio 只问“这个红框是不是你要的做空形态”；HTML 不展示置信度和结果数字。
...
- 已导入任务：`datasets/eth_3m_v10_prebox200/label_studio/tasks.json`
- 二选一界面：`datasets/eth_3m_v10_prebox200/label_studio/label_config.xml`
- 导入验收回执：`datasets/eth_3m_v10_prebox200/label_studio/import_receipt.json`
- 生成器：`scripts/build_eth_3m_v10_prebox200.py`

## Label Studio 后台契约

- 可见问题只有 `is_target = 是 / 不是`；快捷键分别为 `1 / 2`。
- 每张只显示一张 review 图：红框已烙在图内，竖虚线右侧固定展示未来 3 小时。
- 旧 `box / shape / outcome / error_reason / note` 仅隐藏保留，以兼容导入预测和一次未提交草稿；它们不再显示，也不再必填。
- 每个任务仍保留原始 v10 prediction 供审计；200/200 causal PNG 与 200/200 review JPEG 已存在。
- Label Studio：project `53`，标题 `ETH_3m_v10_prebox200_yes_no_20260729`，本机入口 `http://127.0.0.1:8081/projects/53/data`。
...
- API 验收为 200 个任务和 200 个唯一 `data.task_id`；task 001 / 100 / 200 的 causal 与 review 图片均返回 HTTP 200。
- Chrome 实页验收只显示“是 / 不是”、一张 review 图和系统 Submit；旧判断控件均不可见。
...
- 远程反向隧道当前未运行；本次只验证了本机 Label Studio，未自动把弱口令服务暴露到公网。
