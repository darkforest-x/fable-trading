...
> **2026-07-29 更新**：本页顶部新增两块——Kronos 基础模型接入、判断层优化清单。
> 旧内容(2026-07-22 起)保留在下方,当时的判断"瓶颈在检测层"仍然成立,
> 但原因已经变了:不是 tip 出生率≈0,而是**在 owner 自己的标注密度下,
> 因果特征一个金标都定位不到**(`scripts/diag_tip_precision_at_owner_density.py`)。

---

## 【已否决 2026-07-30】Kronos:测过了,不采纳

**结论**:配对贡献 +2.42bp(t=1.13,8/15 折),**置换检验 p=0.0333 未过 0.01 门槛**。
判据是在生成特征之前写死的,不因看到数字而放宽。

```
方案            顶档提升   正折数   ATR对照    超对照
BASE 28+19     +23.49bp  14/15   -19.24bp  +42.73bp
+Kronos 6      +35.61bp  14/15   -11.33bp  +46.94bp
仅 Kronos 6     -9.92bp   0/15    +0.11bp  -10.03bp
配对(同折)    +2.42bp  t=1.13  8/15
置换检验        真实 +35.61bp,打乱中位 +0.50bp,p=0.0333
```

**最有信息量的是「仅 Kronos」那行:单独用它的预测挑单,15 折全部为负。**
它的价格预测对这批候选的盈亏没有可用信息。与论文 RankIC 高 93% 不矛盾——
**RankIC 是排序相关性,和「扣成本后赚钱」是两回事**,本项目 v1 栽过同样的坑。

成本已付:特征生成 2.57 小时 + 半天工程。产物留在 `data/kronos_feats_v10.csv`,
脚本 `scripts/build_kronos_features.py` / `scripts/diag_kronos_feature_value.py`,
将来若换候选池可以直接复用,不必重做工程。

<details><summary>原 TODO 内容(保留备查)</summary>

### Kronos:开源 K 线基础模型,作为判断层的表征来源

**是什么**:清华团队的 `shiyu-coder/Kronos`,AAAI 2026,MIT 协议。
第一个开源 K 线基础模型,45 个交易所、120 亿根 K 线预训练。
两段式:分词器把 OHLCV 量化成分层离散 token → decoder-only Transformer 自回归预测。
...
**为什么可能有用**:我们现在用 28 个手工特征 + 19 个 alpha 因子,今天测出换目标只值
+17.82bp 且置换检验 p=0.32(打不过随机)。Kronos 不设计特征,直接从 120 亿根 K 线学表征。
上下文 512 根,我们的窗口 200 根,对得上。模型小,3060 富余。

**论文成绩**:RankIC 比最好的时序基础模型高 93%、比非预训练基线高 87%;
波动率 MAE 低 9%;合成 K 线保真度高 22%。
⚠️ **RankIC 是排序相关性,不是收益**——本项目 v1 就栽在"AUC 0.59 照样亏钱"。

**接法**:对每个候选取信号 bar 前 400 根,让 Kronos 预测后 **12** 根,
把预测的累计涨跌当作判断层的新特征。

**成本已实测**(2026-07-30,M4 MPS,Kronos-small,`scripts/kronos_batch_bench.py`):

| 批大小 | 预测长度 | 每条 | 25,602 条需要 |
|---|---|---|---|
| 1 | 72 | 7.83s | 55.7h |
| 8 | 72 | 4.51s | 32.1h |
| 8 | 24 | 1.68s | 11.9h |
...
**批量只快 1.7 倍**(MPS 上瓶颈是自回归串行,不是并行度),**真正省时间的是缩短预测长度**
(72→12 根快 10 倍),而我们要的本来就是「会跌多少」不是逐根价格。批 32 已过头。
→ 从「不可行 98 小时」变成「一晚上 5.5 小时」,不占 3060。

**判据(先定死,免得事后找理由)**:
- 顶档提升必须 **> +17.82bp**(现有最好)
- **置换检验 p < 0.01**(现有的 p=0.32,没过)
- **必须带匹配随机对照**(同币 × 同月 × 同 ATR 桶)

**已知的天花板**:匹配对照显示检测器的因果贡献只有 **+8.97bp**,往返成本 10bp。
Kronos 不会凭空造 alpha——信息只值 9bp,榨干也就 9bp。
且它是预测价格的,**帮不了检测层**。

**先决条件**:必须在 **v10 候选池**上试,不是老池(同源纪律)。
...
按"已验证 / 待验证 / 已否决"分三档。全部要在 v10 池上重做——
今天所有判断层结论都基于 `owner_side_short_tip_v1b` 产的 25,602 行老池。

### 已验证有效(但要在 v10 池复验)

| 项 | 证据 | 值多少 |
|---|---|---|
| **目标从二分类改回归** | 三个独立测量一致:2×3 拆解 +17.82bp、配对检验 +21.46bp(t=3.21,13/15 折,p=0.0074)、九种出场里 tponly +18.09bp | **+18~21bp** |
| **出场改「只止盈无止损」** | 九种出场同候选对照,因果超额最高 | **+7.5bp**(vs 现行 TP5/SL2) |

**为什么二分类是错的**:ATR 五分档胜率 36.2~37.7% 持平,每笔净差 5 倍
→ 边在**幅度**不在**胜率**,而分类器学的正是没信息的那一维,AUC 0.4962 是必然。

### 待验证

- **Kronos 表征**(见 TODO-A)
- **动态仓位**:按预测幅度调仓而非等权。前提是先有可信的幅度预测——现在还没有
- **CPCV 禁运按时间而非行数**:现行 72 行 ≈ 12.5 小时,而标签窗口 18 小时。
  已测:禁运加到 7 天结论不变,但脚本里默认值仍是行数,应改成时间
- **新数据维度**:资金费率/持仓量/主动买卖量已开始采集(`src/data/fetch_altdata.py`,
  99 币 × 4 源)。**历史深度不够回测**(资金费率 3 月、其余 30 天),
  约 3 个月后可用。**必须每天跑,断一天是永久缺口**
...
|---|---|
| CV 变异系数 | 无区分力,p=0.35 |
| FFD 分数阶差分 | 技术成功但三个特征全不显著 |
| 回踩入场 / 结构止损 / 移动止损 / 均线反转出场 | 全部差于固定障碍 |
| 「趋势出场」(收回均线束才平) | 九种里倒数第三,+9.40bp,比现行还差 |
| 止损下限单调扫描 | 单调到边界=参数在被移除不是被优化,加对照组才看清 |
| PPO-GRU 强化学习 | 样本 1748,PPO 需 10⁵~10⁶ 步;且没有边可供优化 |

### 一个必须记住的方法论

**任何结果表都要带匹配随机对照。** 这个池子 +26.91bp 里 +17.15bp 是做空 beta,
检测器只值 +8.97bp。**置换检验抓不到这个**——它打乱行,而 beta 在每一行下面都一样。
→ `docs/learnings/pool-internal-metrics-cannot-see-beta.md`

---

## 【TODO-C】升级检测器时先测「开火重合度」

现在每次换检测器都要盲等 16 小时重造候选池。应该先花几分钟测:

```
新旧检测器在同一批 K 线上跑,统计有多少 bar 两个都开火
> 80%   老池可先用,风险低
50~80%  混用并标注
< 50%   必须重造
```

**脚本已就位**:`scripts/diag_detector_fire_overlap.py`(用 Jaccard 而非「A 被覆盖的比例」,
否则开火多的一方靠覆盖一切就能得高分)。

**首次实测 v9 vs v10**(2026-07-30,10 币 × 1200 根,16 分钟):

```
v9  密度 54.0 条/币·月      v10 密度 27.6 条/币·月
Jaccard 重合度 8.6%
v9 的开火被 v10 覆盖 12.0% · v10 的开火被 v9 覆盖 23.5%
共同开火的置信度相关 +0.040
```

**v10 不是 v9 的改进版,是一个几乎不同的检测器** —— 它的开火里 76% 是 v9 完全没看到的。
连两个都开火的同一根 bar 上,信心值也几乎不相关(+0.040),
说明学到的判据不同,不只是阈值不同。
→ **必须重造候选池,这不再是纪律要求而是测量结论。**
