...
当前状态（2026-09-22 18:48北京时间观察）：3060已开始A组第1/40轮，GPU利用率100%，显存6023MiB。WMI进程9416，远端预检通过且冻结audit.json字节未变。新训练尚未完成，没有提前报告效果。后续训练、两套评估和结果回传使用普通Python进程衔接；不通过反复唤醒模型报告未变化的状态。应用目标卡仍paused，工具不支持resume，已提示Owner界面恢复；该限制不阻止本轮执行。

## 2026-09-22 A完成后的评估冲突诊断

A已完成40轮、B继续训练。原生val mAP50仅0.08624，而逐图命中174/175；单变量诊断发现validation pad=.5增加左右各16像素，训练/predict pad=0无此横向补边。同权重/图片/框、同标准AP、全部val350/test320，仅统一padding：CPU val mAP50由0.08650恢复0.99465，test由0.05496恢复0.99500。不是重训所得，不能解释为新增泛化或收益能力。原生失败指标保留，模型对补边脆弱仍未解决；A训练框横向位置仅两种，需另外验位置稳健性。完整原因、可逆单图对照、风险与复现见`analysis/p1_ma_morphology_padding_diagnosis_20260922.md`。B及原自动收集流程未修改，后续解读必须并列保留原始和对齐的形态指标。
