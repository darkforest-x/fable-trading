...
官方构建拒绝覆盖既有快照、扫描结果和交付回执。从零复现必须使用新的实验目录，不能删除本轮
证据后冒充第一次运行。

## 下一步

本轮不需要人工审核才能完成本地结果。Telegram 发送启动后，Owner 明确改为“不需要发 TG”；
进程已立即中断。中断前已经发送总览和前 7 张信号图，共 8 个文件；剩余 17 张信号图、ZIP 与
HTML 共 19 个文件未发送，回执已标记为 Owner 取消并禁止自动续传。若后续要比较 full40-960 或
full40-1280，应继续使用同一批 30 个信号日和相同渲染/合并规则，单独登记新的 holdout 消费次数。
