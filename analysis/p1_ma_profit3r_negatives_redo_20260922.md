...
独立评估代码核对：正负事件均沿规则已知候选方向取模型预测框最大confidence，未使用正例才有的GT框IoU或未来收益来排序。因此它衡量给定规则候选及方向时的排序，不能解释为YOLO独立全市场扫描并自行选择方向的收益。固定confidence0.25触发、IoU0.5命中与连续分数全池top10分别报告；匹配随机收益在top10选择后才接入。最终模型以实际`best.pt` SHA绑定；CSV最高mAP50-95行只是曲线摘要，不能据此推断所选checkpoint的确切epoch。
