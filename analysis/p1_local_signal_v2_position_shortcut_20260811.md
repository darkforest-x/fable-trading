...
修复 builder 入库后，独立数据集 `local_signal_v2_p1_causal_blank_w30_v3` 曾通过九道机械
P0 门，但 Owner 随后目视发现：它只改变框在白色画布上的 X，框相对真实 K 线仍固定在内容
末端。该 P0 属于坐标系误判，数据已标记 `training_eligible=false`，没有启动训练。
...
被否决的 blank-only 位置臂共 4,776 样本：train 2,030 正 + 2,030 负，val 358 正 + 358 负。框中心范围
...
保持 `8357528442c9e0c8e1d63a2fb2b4497f1cb96d51462bea7b4c5a0af008a594c8`。这些数字仅证明
画布布局可复现，不再代表数据可训练。
...
2. blank-only 位置臂：机械 P0 曾通过，但 Owner 目视否决，禁止训练；
3. Owner 已授权恢复真正 Stage A：改变真实连续 K 的 `crop_start_bar`，先审左/中/右预览；
4. Stage A 只作表征预训练，最终仍需严格因果 Stage B 与连续 tip 密度回放。
