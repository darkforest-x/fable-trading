...
- YOLO / auto_label 与判断主线现均为 **SMA/EMA 20/60/120**。
- 全量验证集 0.3% 成本 PF 0.636；maker 0.06% PF 1.072，均未过 1.3。
- 统一解决的是架构一致性，不会自动制造足够厚的 alpha。
...
### 原因 6：均线栈已统一，但 mAP 不是收益

检测与判断现都使用 20/60/120；即使视觉识别继续变好，也仍需 LightGBM 排序、成本与
前向成交共同验证，不能从 mAP 直接推导收益。
