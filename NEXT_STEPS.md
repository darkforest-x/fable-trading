...
## 当前自动链快照（2026-07-14，额度耗尽前固化）

无人值守自动完成的链条（谁接手都不用碰，只看 TG）：
1. **v5 双底座训练**（queue14，GPU，~今天下午出分）→ TG 推学习曲线第 5 点
   （参照：0.35→0.37→0.457→0.511，规则基线 0.45 已破，终点线 owner 自一致性 0.88）；
2. **queue14b 自动接力**：v5 赢家晋升 models/owner_best.pt → 生成 round5
   模型预标任务包（含迷惑区采样）→ 自动上架 Label Studio → 视觉侦察换新模型
   → 部署 VPS → TG 通知 owner 可继续标注；
3. **视觉侦察循环**（30 分钟/班）：增量数据 → 检测器扫合约右缘 → TG 警报 +
   看板 /scout.html 画廊；
4. **每日 8 点定时**：数据增量 → 前向跟踪（crypto 口径）→ TG 日报；
5. **LS 公网隧道**（VPS:8080）持续可用，owner 上班可远程标注。

下个会话开场提示：fable-advisor 插件已启用（重启生效）——按"架构师-实现者"
分工，判断留给会话模型，实现路由给 grok-implementer（CLI 已装）；BUG 工单
和 P2 工程活优先走便宜车道。

## BUG 工单（07-13 排查，指派 Codex）

1. `scripts/fiftyone_label_audit.py`、`scripts/export_yolo_preds_for_audit.py` 等
   仍硬编码 `dense_15m_full`——加 `--dataset` 参数（round-4 起主数据集为 dense_swap_v1）；
2. 各 queue 脚本的 F1 评估改为 `from src.detection.owner_eval import evaluate_owner_f1`
   （四份手写拷贝退役，防漂移）；
3. 验证 `src/judgment/forward.py` 出口统计是否按 `is_stockish` 分渠道
   （scan 层已打标，汇总层需跟进）；数字对不上先查这里。
...
读取 `data/forward_log.csv`，按 maker-filled closed 样本展示 0/100 裁决进度、PF、胜率、
