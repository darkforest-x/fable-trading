...
def _features(index, *, exits=(), last_side=-1, last_run=7, known=True):
    n = len(index)
    ...
    return pd.DataFrame({"strong_side": side, "strong_streak": streak,
                         "last_strong_side": np.full(n, last_side, dtype=int),
                         "last_strong_run": pd.array([last_run] * n, dtype="Int64"),
                         "counter_known": np.full(n, known, dtype=bool)}, index=index)
...
def test_injected_exact_seventh_exits_at_next_open_then_allows_serial_reentry():
    frame = _frame()
    frame.loc[frame.index[13], ["open", "high", "low", "close"]] = [99.0, 100.0, 98.5, 99.0]
    fired = np.zeros(len(frame), dtype=bool); fired[[10, 13]] = True
    got = subject.positions(frame, _facts(len(frame)), fired, minutes=60, tick=.01,
                            rsi_exit_enabled=True, rsi_features=_features(frame.index, exits=(12,)))
    first, second = got[10], got[13]
    assert first["exit_reason"] == "rsi_seventh_reverse_next_open"
    assert first["exit_time_ms"] == int(frame.index[13].value // 1_000_000)
    assert first["exit_time_precision"] == "bar_open"
    assert first["rsi_exit_trigger_close_ms"] == int((frame.index[12] + pd.Timedelta(hours=1)).value // 1_000_000)
...
def test_stop_gap_still_beats_injected_rsi_and_no_profit_filter_is_applied():
    frame = _frame()
    frame.loc[frame.index[13], ["open", "high", "low", "close"]] = [97.5, 98.0, 97.0, 97.5]
    fired = np.zeros(len(frame), dtype=bool); fired[10] = True
    got = subject.positions(frame, _facts(len(frame)), fired, minutes=60, tick=.01,
                            rsi_exit_enabled=True, rsi_features=_features(frame.index, exits=(12,)))[10]
...
def test_baseline_default_ignores_injected_rsi_and_active_card_reports_pending_only_at_last_close():
    frame = _frame()
    fired = np.zeros(len(frame), dtype=bool); fired[10] = True
    features = _features(frame.index, exits=(len(frame) - 1,))
...
def test_closed_rsi_metadata_uses_its_trigger_prefix_not_future_counter_state():
    frame = _frame()
    fired = np.zeros(len(frame), dtype=bool); fired[10] = True
    features = _features(frame.index, exits=(12,))
    # A later color/run must not leak into the close already executed at 13's open.
    features.loc[frame.index[14]:, "last_strong_side"] = 1
    features.loc[frame.index[14]:, "last_strong_run"] = 99
    full = subject.positions(frame, _facts(len(frame)), fired, minutes=60, tick=.01,
                             rsi_exit_enabled=True, rsi_features=features)[10]
    short_frame, short_features = frame.iloc[:14], features.iloc[:14]
    short = subject.positions(short_frame, _facts(len(short_frame)), fired[:14], minutes=60, tick=.01,
