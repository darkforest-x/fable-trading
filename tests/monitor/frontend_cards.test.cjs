...
test("candidate stages remain separate, errors stay visible and notification receipts cannot leak", () => {
  const { client, get } = harness();
  client.state.candidates = [candidate("pending"), candidate("error", "error"), candidate("expired", "expired"), candidate("invalid", "invalidated"), candidate("passed", "confirmed"), candidate("disabled-five", "disabled", {timeframe: "5m"})];
  ...
  client.renderSignals();
  assert.deepEqual(ids(get("signal-rows")), ["pending", "error", "expired", "invalid", "passed", "disabled-five"]);
  assert.match(get("signal-rows").innerHTML, /等待已到期|结构失效|模型已通过/);
  assert.match(get("signal-rows").innerHTML, /周期已关闭/);
  assert.match(get("signal-rows").innerHTML, /该周期已关闭 · 不再推送/);
});
...
test("30m signal and watch filters isolate their own cards without launching an app", () => {
  const { client, get, document, networkRequests } = harness({ initialScope: "direct" });
  enableTwoStage(client);
  client.state.directSignals = [candidate("half-hour", "pending", { timeframe: "30m" }), candidate("hour")];
  client.state.markets = [market("SOPH-USDT-SWAP", "30m"), market("ETH-USDT-SWAP", "1H")];
  document.querySelectorAll("[data-timeframe]").find((button) => button.dataset.timeframe === "30m").dispatch("click");
  assert.deepEqual(ids(get("signal-rows")), ["half-hour"]);
  document.querySelectorAll("[data-watch-timeframe]").find((button) => button.dataset.watchTimeframe === "30m").dispatch("click");
  assert.equal(client.state.watchTimeframe, "30m");
