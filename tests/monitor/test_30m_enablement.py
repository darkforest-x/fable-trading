...
def legacy_five_minute_event(monkeypatch, *, symbol="LEGACY-USDT-SWAP"):
    """Construct an actually aligned old proof without enabling its new policy."""
    with monkeypatch.context() as temporary:
        temporary.setitem(TIMEFRAMES, "5m", 300_000)
        return model_event(timeframe="5m", wait=0, close=NOW - 60_000,
...
def test_withdrawal_retires_only_pending_bark_and_model_work_and_is_idempotent(tmp_path, monkeypatch):
    store = Store(tmp_path / "monitor.sqlite")
    for protocol in (DIRECT_POLICY, MODEL_PROTOCOL):
        store.set_meta("notification_timeframe:" + protocol + ":5m", {"activated_ms": 12345})
    ...
    for index, status in enumerate(("pending", "sent", "unknown", "sending")):
        event = legacy_five_minute_event(monkeypatch, symbol=f"OLD{index}-USDT-SWAP")
    ...
    active = model_event(timeframe="30m", wait=0, close=NOW - 60_000)
    ...
    legacy = {e["id"]: e for e in store.list_candidates(timeframe="5m")}
    ...
    assert store.list_candidates(timeframe="5m", pending_only=True) == []
    assert len(store.list_candidates(timeframe="30m", pending_only=True)) == 1
...
def test_sender_rechecks_withdrawn_raw_and_yolo_even_without_startup_migration(tmp_path, monkeypatch):
    ...
    store = Store(tmp_path / "monitor.sqlite")
    event = legacy_five_minute_event(monkeypatch)
    for protocol in (DIRECT_POLICY, MODEL_PROTOCOL):
        store.activate_bark_policy(0, protocol=protocol, retire_obsolete=False)
        store.set_meta("notification_timeframe:" + protocol + ":5m", {"activated_ms": 0})
    ...
    def forbidden(*args, **kwargs):
        pytest.fail("withdrawn five-minute event reached network sender")
...
def test_withdrawn_period_never_enters_model_or_current_market_grid(tmp_path, monkeypatch):
    ...
    gate.submit(raw["symbol"], "5m", [])
    assert not gate._queue
    gate.process(raw["symbol"], "5m", [])  # Does not index the removed duration.
    ...
    assert {r["timeframe"] for r in store.list_markets()} == {"5m", "30m"}
...
def test_startup_withdraws_before_any_delivery_or_model_worker_starts(tmp_path, monkeypatch):
    ...
    def thread(*, target, name, daemon):
        def start():
            ...
            assert store.list_candidates(timeframe="5m")[0]["model"]["status"] == "disabled"
