"""Synthetic V18 notebook wiring; accounting-domain tests belong to verifier."""
...
from yoyo.evaluation.hourly_impulse_failed_confirm_notebook import (
...
spec=importlib.util.spec_from_file_location("_v18_notebook_stub_dependency",Path(__file__).with_name("verify_hourly_impulse_failed_launch_v17.py"))
...
def verify_tables(tables, summary, *, expected_counts=(251,462,154)):
    ...
    assert summary["experiment_id"].endswith("-v18")
    ...
    return {"counts":{"cases":251,"controls":462,"matched":154,"unmatched":97},
            "effects":summary["effects"],"accounting":{"serial_recomputed":True,
                ...
                "failed_launch_exits":{"baseline/case":140,"baseline/control":300,"candidate/case":100,"candidate/control":200},
                "confirmation_lifecycle":{"baseline/case":{"created":0,"cancelled":0,"terminated":0,"confirmed":0},
                    "baseline/control":{"created":0,"cancelled":0,"terminated":0,"confirmed":0},
                    "candidate/case":{"created":170,"cancelled":50,"terminated":20,"confirmed":100},
                    "candidate/control":{"created":350,"cancelled":100,"terminated":50,"confirmed":200}}},
...
'''
    dependency_source='''import importlib.util
from pathlib import Path
...
dependency=importlib.util.module_from_spec(spec)
spec.loader.exec_module(dependency)
PINNED_DEPENDENCY=dependency.PINNED_DEPENDENCY
'''
    hashes={}
    for name in VERIFIER_FILES:
        path=tmp_path/name;path.parent.mkdir(parents=True,exist_ok=True)
        path.write_text(source if name==VERIFIER_FILES[0] else dependency_source if name==VERIFIER_FILES[1] else "PINNED_DEPENDENCY=True\n")
...
def test_three_cells_15_saved_tables_and_no_raw_inference_claim(tmp_path):
    ...
    assert "快5分钟真实翻色" in text and "V17原净赢家的逐事件路径不变" in text and "不能把冻结20bp的触发门改成30bp" in text
    assert "旧赢家可能转亏" not in text and "原亏损可救回，也可亏得更深" in text
    assert "相对V16旧赢家" in text
    assert "未知不补零" in text and "3小时20分钟→10小时" not in text
    assert "失败退出二次确认" in "".join(result["cells"][0]["source"])
    assert "病例100笔" in "".join(result["cells"][0]["source"])
    assert set(m["verifier_hashes"])==set(VERIFIER_FILES) and len(VERIFIER_FILES)==3
    assert "病例pending创建170次" in "".join(result["cells"][0]["source"])
    assert "取消50次" in "".join(result["cells"][0]["source"])
...
@pytest.mark.parametrize("mutation",[
    ...
    lambda s:s["arms"]["baseline"]["policy"].update(fast_failed_launch_exit=False),
    ...
    lambda s:s["arms"]["candidate"]["policy"].pop("fast_failed_launch_confirmations"),
    lambda s:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=1),
    lambda s:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=True),
    lambda s:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=2.0),
    lambda s:s["arms"]["baseline"]["policy"].update(fast_failed_launch_confirmations=2),
])
...
@pytest.mark.parametrize("old,new,match",[
    ...
    ('"baseline/case":140','"baseline/case":252',"full-exit count"),
    ...
    ('"created":170','"created":171',"Unresolved pending"),
    ('"confirmed":100','"confirmed":99',"Unresolved pending"),
    ('"cancelled":50','"cancelled":True',"Invalid lifecycle"),
])
def test_rehashed_stub_receipt_must_preserve_v18_scope_and_full_population(tmp_path,old,new,match):
    pins=synthetic_evidence(tmp_path);path=tmp_path/VERIFIER_FILES[0]
...
@pytest.mark.parametrize("dependency_index", [1, 2])
def test_all_dependency_hashes_are_checked_before_any_source_is_imported(tmp_path,dependency_index):
    pins=synthetic_evidence(tmp_path);path=tmp_path/VERIFIER_FILES[0]
    path.write_text("raise RuntimeError('MUST NOT IMPORT')\n")
    pins[1][VERIFIER_FILES[0]]=hashlib.sha256(path.read_bytes()).hexdigest()
    (tmp_path/VERIFIER_FILES[dependency_index]).write_text("PINNED_DEPENDENCY=False\n")
    with pytest.raises(ValueError,match="dependency hash"):execute_notebook(build_notebook(*pins),tmp_path)
