"""V18 presentation guards use synthetic summaries, never actual outcomes."""
...
from yoyo.evaluation.hourly_impulse_failed_confirm_report import (
...
def fixture():
    ...
    kwargs=dict(markdown_path="analysis/v18.md",summary_path="experiments/v18/summary.json",
        case_delta_path="experiments/v18/case_delta.csv",generated_at="2026-09-06T00:00:00Z",fixture=True)
...
def test_complete_markdown_sql_counts_and_correct_partial_semantics():
    ...
    assert [b.get("sourceId") for b in blocks]==[None,"v18_summary","v18_case_delta",None,None]
    ...
    source=next(s for s in artifact["sources"] if s["id"]=="v18_case_delta")
    ...
    assert "Waiting may preserve a recovery or worsen a loss" in source["query"]["description"]
    assert "V17's positive-net event paths remain unchanged" in source["query"]["description"]
    assert "V17 nonpositive failed full exit" in source["query"]["description"]
    assert "recovered profitable second open cannot invent a half fill" in source["query"]["description"]
    assert "Profitable first-edge partial realization is not delayed" in source["query"]["description"]
    assert "independently recomputes serial occupancy" in source["query"]["description"]
    assert source["path"]=="experiments/v18/case_delta.csv"
    ...
    assert "identical final exit" not in text and "V17" in text
    ...
    assert paths["research_code"]=="yoyo/evaluation/hourly_impulse_failed_confirm_research.py"
...
@pytest.mark.parametrize("mutation",[
    ...
    lambda s,d,k:s["arms"]["baseline"]["policy"].update(fast_failed_launch_exit=False),
    ...
    lambda s,d,k:s["arms"]["candidate"]["policy"].pop("fast_failed_launch_confirmations"),
    lambda s,d,k:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=1),
    lambda s,d,k:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=True),
    lambda s,d,k:s["arms"]["candidate"]["policy"].update(fast_failed_launch_confirmations=2.0),
    lambda s,d,k:s["arms"]["baseline"]["policy"].update(fast_failed_launch_confirmations=2),
...
def test_presentation_mirrors_frozen_runner_policy_literals_without_importing_strategy():
    path=Path(__file__).resolve().parents[1]/"yoyo/evaluation/hourly_impulse_failed_confirm_research.py"
    assignments={}
