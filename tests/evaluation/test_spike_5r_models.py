"""Regression checks for causal fixed-model Spike 5R scoring."""
...
from yoyo.evaluation import spike_10r_models as legacy
...
def test_future_mutation_cannot_change_earlier_training_or_scores(name):
    ...
    changed.loc[future, "label_gt5"] = ~changed.loc[future, "label_gt5"]
...
def test_feature_matrix_ignores_forbidden_columns_and_preprocessing_is_train_only():
    ...
    changed.loc[:, "label_gt5"] = ~changed.label_gt5
...
@pytest.mark.parametrize("name", models.MODEL_NAMES)
def test_target_change_preserves_the_fixed_feature_and_estimator_contract(name):
    """The versioned study changes only its strict outcome threshold."""
    assert models.MODEL_FEATURES == legacy.MODEL_FEATURES
    assert models.TIMEFRAMES == legacy.TIMEFRAMES
    assert models.SEED == legacy.SEED
    current = models.fit_model(name, _rows())
    prior = legacy.fit_model(name, _rows().rename(columns={"label_gt5": "label_gt10"}))
    assert current.estimator.get_params() == prior.estimator.get_params()
...
def test_scores_are_probabilities_and_serialization_roundtrips(tmp_path, name):
    ...
    assert metadata["target_r"] == 5
    assert metadata["label_column"] == "label_gt5"
