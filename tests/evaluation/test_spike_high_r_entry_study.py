...
from yoyo.evaluation.spike_high_r_entry_study import completed, digest, tag
from yoyo.evaluation.spike_high_r_entry_report import block_test, periods
from yoyo.evaluation import spike_high_r_entry_study as study
...
def test_tag_uses_signal_identity_not_execution_row_number():
    ...
    assert tag(d, "high_r_entry_v2").event_key.tolist() == ["x:14:1", "x:14:-1"]
    with pytest.raises(ValueError, match="duplicate"):
        tag(pd.concat([d, d]), "high_r_entry_v2")
...
def test_auc_direction_and_ties():
    from yoyo.evaluation.spike_high_r_entry_report import auc_score
...
def test_rate_bootstrap_preserves_counts_and_missing_arm_months():
    from yoyo.evaluation.spike_high_r_entry_report import rate_contrast
    d = pd.DataFrame(dict(arm=["baseline"]*4+["high_r_entry_v2"]*2,
