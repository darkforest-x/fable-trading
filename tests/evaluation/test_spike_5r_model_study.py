...
def test_relabel_candidates_is_strict_at_five_and_preserves_source_labels():
    source = pd.DataFrame({
        'valid_entry': [True, True, True, False, True, True],
        'censored': [False, False, True, False, False, False],
        'net_r': [5., 5.00001, 9., 20., np.nan, np.inf],
        'label_gt10': [False, False, True, True, False, False],
    })
    labelled = m.relabel_candidates(source)
    assert labelled.label_gt5.iloc[:2].tolist() == [False, True]
    assert labelled.label_gt5.iloc[2:].isna().all()
    assert source.columns.tolist() == ['valid_entry', 'censored', 'net_r', 'label_gt10']
    assert labelled.label_gt10.equals(source.label_gt10)
...
def test_serial_trigger_replays_all_predeclared_models_not_only_best():
    rows=[dict(period='oof',rule='original_all',precision=.01,gt5=100,recall=1.)]
    rows.extend(dict(period='oof',rule=name,precision=.009,gt5=5,recall=.05) for name in m.model_policies())
    table=pd.DataFrame(rows)
    assert not m.serial_trigger(table)['needed']
    table.loc[1,['precision','gt5','recall']]=[.02,10,.1]
