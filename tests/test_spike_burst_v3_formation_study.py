"""Synthetic contracts for D label denominators, parent economics and Holm4."""
import pandas as pd
import pytest

from yoyo.evaluation import spike_burst_v3_formation_study as study
...
def test_d_cannot_shrink_original_positive_or_large_denominator():
    ...
    row = result[result.arm.eq("formation_gate") & result.stage.eq("early") & result.period.eq("full")].iloc[0]
...
def test_missing_detection_is_error_not_a_smaller_population():
    ...
    detections = detections[~(detections.arm.eq("formation_gate") & detections.stage.eq("early") & detections.event_i.eq(124))]
...
def p_tables():
    ...
    d = pd.DataFrame(dict(arm=["formation_gate"], period=["full"], permutation_p=[.001]))
    return ab, c, d


def test_four_hypotheses_remain_in_holm_family():
    result = study.structural_holm(*p_tables()).set_index("arm")
    assert len(result) == 4
    assert result.loc["formation_gate", "holm_p_four"] == pytest.approx(.004)
    assert result.loc["confirmation_gate", "holm_p_four"] == pytest.approx(.009)
    assert result.loc["reference", "holm_p_four"] == pytest.approx(.08)
    assert result.loc["near_box", "holm_p_four"] == pytest.approx(.08)


def test_missing_historical_hypothesis_fails_closed():
    ab, c, d = p_tables()
    with pytest.raises(ValueError, match="Exactly one frozen"):
        study.structural_holm(ab, c.iloc[:0], d)


def test_missing_p_keeps_conservative_one_in_family():
    ab, c, d = p_tables()
    c.loc[0, "permutation_p"] = float("nan")
    result = study.structural_holm(ab, c, d).set_index("arm")
    assert len(result) == 4 and result.loc["confirmation_gate", "correction_input_p"] == 1
    assert result.loc["formation_gate", "holm_p_four"] == pytest.approx(.004)
    d.loc[0, "permutation_p"] = -1
    with pytest.raises(ValueError, match="within"):
        study.structural_holm(ab, c, d)
...
def test_acceptance_cannot_trade_less_noise_for_missing_launches():
    retention = pd.DataFrame(dict(arm=["formation_gate"], stage=["early"], period=["full"],
        label_drop=[.9], distinct_labels=[1200], large_recall_1=[.2], retention=[.3]))
    trades = pd.DataFrame(dict(arm=["formation_gate"], period=["full"], mean_excess_bp=[10.], holm_p_four=[.2]))
...
def test_original_absolute_label_budget_is_preserved():
    ...
    trades = pd.DataFrame(dict(arm=["formation_gate"], period=["full"], mean_excess_bp=[10.], holm_p_four=[.005]))
    result = study.assessment(retention, trades)
