"""Synthetic SQL arithmetic/unknown checks before V32 report materialization."""
...
path = Path(__file__).resolve().parents[1]/"yoyo/evaluation/hourly_impulse_volume_wave_economic_report.py"
spec = importlib.util.spec_from_file_location("_report_v32", path)
report = importlib.util.module_from_spec(spec)
spec.loader.exec_module(report)


def test_policy_cost_and_gross_decomposition_same_denominator():
    cases = pd.DataFrame(dict(horizon_hours=[4,4,4], wave_gate_state=["accepted", "abstain", "unknown"],
