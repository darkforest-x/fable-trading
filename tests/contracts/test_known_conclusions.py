...
def test_owner_yolo_predictions_did_not_preserve_training_semantics(registries):
    row = next(
        e
        for e in registries.experiments
        if e.experiment_id
    ...
    assert "41/43 were out of training spec" in row.result
    assert "9,578/10,000" in row.result
    assert row.production_eligible is False
