"""Package the complete reviewed V21 narrative with source-backed support bars.
...
REPORT = "analysis/p1_btcusdtp_hourly_breadth_v21_20260906.md"
TITLE = "External Rank Pressure"
...
def main():
    ...
    sources = [
        {"id": "report", "label": "V21 · 完整方法、结果与限制", "path": REPORT},
        {"id": "support", "label": "V21 · 原始713个自身外部均值方向状态", "path": REL+"/results/entry_context.csv.gz",
         "query": {"sql": QUERY, "engine": "sqlite", "language": "sql",
            "tables_used": ["entry_context"], "description": "按原始半年度汇总各自外部均值门，不读取收益",
            "filters": ["2023--2024 original251 cases and462 fixed own controls"],
            "executed_at": pd.Timestamp.now(tz="UTC").isoformat()}},
        {"id": "summary", "label": "V21 · 冻结汇总", "path": REL+"/results/summary.json"},
        {"id": "verification", "label": "V21 · 独立四币小时分数复核", "path": REL+"/results/independent_verification.json"},
        {"id": "delta", "label": "V21 · 全251机会收益增量", "path": REL+"/results/case_delta.csv.gz",
    ...
    chart = {"id": "support", "type": "bar", "title": "各半年度获准的原始 K1 机会",
        "description": "2023—2024年；柱为外部均值同向的机会数，参考线为每半年度最低12笔，不代表收益",
