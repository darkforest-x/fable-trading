#!/usr/bin/env python3
"""Select and audit EMA-weakness profit harvesting for ETHUSDT.P 15m.

Favourable excursions at +2/+4/+8/+12 signal ATR earn up to four release
slots. No position is reduced while price closes on the trend side of EMA30.
One earned slot is scheduled after each completed weak close and filled at the
next bar open. A release never changes the disaster or SMA60/ATR runner stop.
Selection changes only the fraction per slot on 2023--2024. Repository holdout
...
EXPERIMENT_ID = "exp-ethusdtp-15m-weakness-harvest-preholdout-20260905-v7"
...
def resolve_weakness_harvest(
    ...
    *,
    ...
    """Release earned tranches on EMA30 weakness without moving the stop.

    Reads OHLC, ATR, ``reference_ma`` (EMA30), and ``trend_ma`` (SMA60) only
    from entry through the frozen horizon. All release decisions use completed
    ...
    harvest = config["weakness_harvest"]
    ...
    result = {
        "resolved": True,
        "policy": "ema30_weakness_harvest_sma60_runner",
        "outcome": outcome,
        "exit_i": exit_i,
...
def selection_phase(config: dict[str, Any]) -> dict[str, Any]:
    ...
    write_csv(
        fold_table(selected, folds).assign(policy="weakness_harvest_v7"),
...
def audit_phase(config: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "paired_candidate_minus_baseline": {
            ...
            "signflip_p": float(
                signflip_p(comparison["delta"], resamples=100_000, seed=90551)
            ),
        },
        "matched_random": {
            ...
            "signflip_p": (
                float(signflip_p(excess, resamples=100_000, seed=90552))
                if len(excess)
                else np.nan
            ),
        },
    ...
    write_csv(
        fold_table(candidate, folds).assign(policy="weakness_harvest_v7"),
