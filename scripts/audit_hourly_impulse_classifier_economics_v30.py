"""Independent saved-number V30 audit: stdlib joins/aggregation, no yoyo import.
...
E = Path("experiments/active/exp-btcusdtp-1h-classifier-economics-preholdout-20260907-v30")
V24 = Path("experiments/active/exp-btcusdtp-1h-fixed-clock-preholdout-20260907-v24/results")
V29 = Path("experiments/active/exp-btcusdtp-1h-classifier-support-preholdout-20260907-v29/results")
...
def run(root=ROOT, *, write_receipt=True):
    ...
    contexts = {kind: {r["event_id"]: r for r in rows(root/V29/(kind+"_context.csv.gz"))} for kind in ("case", "control")}
