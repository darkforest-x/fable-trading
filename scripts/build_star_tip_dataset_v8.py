...
OUT = PROJECT / "datasets/dense_owner_short_star_tip_v8"
...
BOX_BARS = 10                   # owner-set 2026-07-27: their boxes are ~10 bars
...
def main() -> int:
    ...
    def emit_positive(sym: str, cut: int, width: int, tag: str, force: bool = False) -> bool:
        ...
        try:
            ...
            # v8: fixed width, owner's call. Measured across 244 of their boxes
            # the width is flat at 9-13 bars regardless of how tight the bundle
            # got: every adaptive rule scored ~0.80 median error against the
            # owner while a fixed 10 scored 0.17. They frame a constant view and
            # let density decide WHETHER to draw, not how wide.
            box = segment_to_bbox(sub, DenseSegment(start=max(0, t1 - BOX_BARS + 1),
                                                    end=t1), tf)
            if box is None:
                skips["box"] += 1
    ...
    meta = {"generated_at": pd.Timestamp.utcnow().isoformat(), "git": _git(),
            ...
            "owner_pattern": "v8: 框宽固定 10 根(owner 定); owner 空头框一律收为正样本; 方向按 star_side 判",
