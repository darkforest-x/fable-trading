...
function decide(value){const item=currentItem(),draft=draftFor(item),g=geometry(draft),before=filtered(),beforeIndex=before.findIndex(x=>x.event_id===item.event_id);if(value==='KEEP'&&!g){alert('KEEP 需要先明确选择确认根数、核心宽度和完整输入窗。');return}snapshot();state.answers[item.event_id]={...identityBase(item),decision:value,reviewed_at:new Date().toISOString(),note:draft.note||null,...(value==='KEEP'?g:nullGeometry())};save();if($('autoNext').checked){const after=filtered(),still=after.findIndex(x=>x.event_id===item.event_id);if(after.length){const nextIndex=still>=0?Math.min(still+1,after.length-1):Math.min(Math.max(beforeIndex,0),after.length-1);state.cursor_id=after[nextIndex].event_id}else state.cursor_id=item.event_id;save();render()}else render()}
