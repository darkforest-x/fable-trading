"""Saved V17 distribution diagnostics; no raw prices, filtering or new inference.
...
def load_pinned_utility(path=UTILITY):
    ...
    spec = importlib.util.spec_from_file_location("_v17_pinned_assumption_checks", path)
    ...
    for name in COLUMNS:
        ...
        if n >= 3:
            with warnings.catch_warnings(record=True) as emitted:
                ...
                observed = normality_check(x.copy(), name="V17 "+name+" (bp)", plot=False)
...
def run(root=ROOT):
    """Read only fixed saved V17 inputs and exclusively create one diagnostic."""
