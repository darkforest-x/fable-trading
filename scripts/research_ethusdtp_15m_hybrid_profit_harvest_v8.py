#!/usr/bin/env python3
"""Select and audit hybrid partial profit harvesting for ETHUSDT.P 15m.

The +2/+4 signal-ATR stages bank immediately. The +8/+12 stages only earn
slots and are released one per completed adverse-side EMA30 close at the next
bar open. No profit event changes the disaster or SMA60/ATR runner stop.
Selection changes only fraction per stage on 2023--2024. Repository holdout
rows are never parsed.
"""
...
EXPERIMENT_ID = "exp-ethusdtp-15m-hybrid-profit-harvest-preholdout-20260905-v8"
...
def resolve_hybrid_harvest(
    frame: pd.DataFrame,
    event: Mapping[str, Any],
    config: Mapping[str, Any],
    *,
    fraction_per_stage: float,
) -> dict[str, Any]:
    """Bank early targets and defer late targets until EMA30 weakness.

    Reads OHLC, ATR, ``reference_ma`` (EMA30), and ``trend_ma`` (SMA60) only
    from entry through the frozen horizon. Late release decisions use completed
    close ``t`` and execute at open ``t+1``. A gap beyond the active stop is
    resolved before a pending release; otherwise the open release precedes any
    later intrabar stop.
    """

    if not 0.0 < fraction_per_stage < 0.25:
        raise ValueError(
            "fraction_per_stage must leave a positive runner after four stages"
        )
    execution = config["frozen_execution"]
    harvest = config["hybrid_harvest"]
    levels = list(map(float, harvest["slot_levels_atr"]))
    immediate_count = int(harvest["immediate_stage_count"])
    ...
    for i in range(entry_i, end_i + 1):
        ...
        if pending_release and partial_hits < earned_slots:
            realized_gross += (
                fraction_per_stage * direction * (open_price / entry - 1.0)
            )
            remaining -= fraction_per_stage
        ...
        while earned_slots < len(levels) and excursion_atr >= levels[earned_slots]:
            slot = earned_slots
            earned_slots += 1
            if slot < immediate_count:
                target = entry + direction * levels[slot] * signal_atr
                realized_gross += (
                    fraction_per_stage * direction * (target / entry - 1.0)
                )
                remaining -= fraction_per_stage
                release_prices.append(target)
                partial_hits += 1
                immediate_hits += 1
        ...
        weak_close = direction * (close - float(frame.loc[i, "reference_ma"])) <= 0.0
        close_is_net_profitable = direction * (close / entry - 1.0) > cost
        if (
            earned_slots > partial_hits
            and not pending_release
            and weak_close
    ...
    result = {
        "resolved": True,
        "policy": "hybrid_profit_harvest_sma60_runner",
        "outcome": outcome,
        "exit_i": exit_i,
        ...
        "bank_total_fraction": 4.0 * fraction_per_stage,
        "fraction_per_stage": fraction_per_stage,
        "step_atr": np.nan,
...
def selection_phase(config: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline = replay(setups, frame, config, fraction_per_stage=None)
    ...
    for fraction in map(float, config["selection"]["fraction_per_stage_candidates"]):
        events = replay(setups, frame, config, fraction_per_stage=fraction)
    ...
    write_csv(
        fold_table(selected, folds).assign(policy="hybrid_harvest_v8"),
    ...
    receipt = {
        ...
        "selected_params": {
            ...
            "slot_levels_atr": config["hybrid_harvest"]["slot_levels_atr"],
        },
        "source": quality,
        "raw_pairs": len(pairs),
    ...
    *,
    ...
    for event in candidate.to_dict("records"):
        ...
        for assignment, control_i in enumerate(choices[:required]):
            ...
            result = resolve_hybrid_harvest(
                frame, control_event, config, fraction_per_stage=fraction
            )
...
def audit_phase(config: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline = replay(setups, frame, config, fraction_per_stage=None)
    candidate = replay(setups, frame, config, fraction_per_stage=fraction)
    ...
    summary = {
        ...
        "paired_candidate_minus_baseline": {
            ...
            "signflip_p": float(
                signflip_p(comparison["delta"], resamples=100_000, seed=90561)
            ),
        },
        "matched_random": {
            ...
            "signflip_p": (
                float(signflip_p(excess, resamples=100_000, seed=90562))
                if len(excess)
                else np.nan
            ),
        },
    ...
    write_csv(
        fold_table(candidate, folds).assign(policy="hybrid_harvest_v8"),
