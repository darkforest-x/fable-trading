...
def main():
    ...
    parts.append('4h共同窗口ICT只剩1笔，无法配满5个同条件随机入场，完整匹配数为0，随机均值和p值不可用；较长历史也仅5笔、完整匹配2笔。其中一笔净+4.98R占总净+6.17R约80.8%。不能把缺失的统计量说成“检验通过”，也不能用100%胜率给4h排名。\n')
    parts.append('15min共同窗口删除14笔全天旧成交（净合计+3.60R），又因释放占仓新增1笔（−1.41R），故总净R由27.75降至22.74。月度ICT净R为1月−1.95、2月+13.45、3月+10.42、4月+0.82；最大一笔+10.05R，约占总额44.2%。过滤提升近期平均交易表现，不能等同于每个月或所有行情都有效。\n')
    parts.append('独立只读复核通过：24组交易统计、48组现金账本、24组基线parity与104个结果文件哈希一致；所有ICT成交都在规定入场时段，退出中没有翻身规则。统计最大误差约4.4e-15，现金恒等式最大残差1.09e-12U。15min共同窗前三个赢家贡献净总额约93.2%，收益仍集中；这是计算复现证据，不是稳定盈利认证。\n')
