...
OUT = PROJECT / "datasets/dense_owner_short_star_tip_v7"
...
def main() -> int:
    ...
    def emit_positive(sym: str, cut: int, width: int, tag: str, force: bool = False) -> bool:
        ...
        try:
            ...
            run0 = dense_run_start(fast, trough)
            span = max(2, min(anchor - run0 + 1, WINDOW - 1))
            box = segment_to_bbox(sub, DenseSegment(start=max(0, t1 - span + 1), end=t1), tf)
            if box is None:
                skips["box"] += 1
    ...
    meta = {"generated_at": pd.Timestamp.utcnow().isoformat(), "git": _git(),
            ...
            "owner_pattern": "v7: 框宽=密集run长度; owner 空头框一律收为正样本; 方向仍按 star_side 判",
