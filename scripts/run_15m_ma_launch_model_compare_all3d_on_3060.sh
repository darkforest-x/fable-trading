...
start() {
  ...
  local commit
  commit=$(source_commit)
  FABLE_SSH_DEADLINE=60 "${SSH_PS[@]}" "$HOST" "\$root='$REMOTE'; if (-not (Test-Path -LiteralPath \$root)) { throw 'remote bundle is not staged' }; if (Test-Path -LiteralPath (Join-Path \$root 'scan.exit')) { throw 'refusing to overwrite existing scan.exit' }; \$runner = Join-Path \$root 'scripts/windows/run_model_compare_all3d_scan.ps1'; \$stdout = Join-Path \$root 'scan.stdout.log'; \$stderr = Join-Path \$root 'scan.stderr.log'; \$command = \"cmd.exe /c powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File \$runner -Root \$root -SourceCommit $commit -BatchSize $BATCH_SIZE > \$stdout 2> \$stderr\"; \$created = Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{CommandLine=\$command}; if (\$created.ReturnValue -ne 0) { throw ('WMI create failed ret=' + \$created.ReturnValue) }; [System.IO.File]::WriteAllText((Join-Path \$root 'scan.pid'), (\$created.ProcessId.ToString() + [Environment]::NewLine), [System.Text.Encoding]::ASCII); Write-Output ('pid=' + \$created.ProcessId + ' source_commit=$commit batch=$BATCH_SIZE')"
}

status() {
  say "remote scan status"
  FABLE_SSH_DEADLINE=45 "${SSH_PS[@]}" "$HOST" "\$root='$REMOTE'; if (-not (Test-Path -LiteralPath \$root)) { throw 'remote bundle is not staged' }; \$exit = Join-Path \$root 'scan.exit'; \$pidPath = Join-Path \$root 'scan.pid'; if (Test-Path \$exit) { Write-Output ('scan.exit=' + (Get-Content \$exit -Raw).Trim()) } elseif (Test-Path \$pidPath) { \$scanPid = [int](Get-Content \$pidPath -Raw); \$process = Get-Process -Id \$scanPid -ErrorAction SilentlyContinue; Write-Output ('scan.pid=' + \$scanPid + ' running=' + [bool]\$process) } else { Write-Output 'not started' }; foreach (\$name in @('scan.stdout.log','scan.stderr.log')) { \$logPath = Join-Path \$root \$name; Write-Output ('--- ' + \$name + ' ---'); if (Test-Path \$logPath) { Get-Content \$logPath -Tail 30 } else { Write-Output 'missing' } }"
}
