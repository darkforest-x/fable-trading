...
def run(planpath):
    ...
    doc+=f'<div class="panel"><h2>{len(matches)} 个去重命中</h2><table><thead><tr><th>周期</th><th>实际覆盖币种</th><th>进入严格评分</th><th>去重命中</th><th>多 / 空</th></tr></thead><tbody>{rows}</tbody></table><p class="sub">覆盖池为本地 Binance / OKX USDT 永续历史数据；同币优先最新可见覆盖、再优先 Binance。不是历史所有已退市币种全集。原规则来自15分数据，其他周期为保持参数不变的探索性回放。<br>源错误：{len(source_errors)}；“未通过”和WIF全貌均不计入命中。数据缺口详见下载文件。</p><a href="results.csv">下载完整严格评分结果 CSV</a> · <a href="coverage.json">数据覆盖</a> · <a href="source_errors.json">缺失与错误</a></div>'
    ...
    for name,value in [('coverage.json',coverage),('source_errors.json',source_errors),('manifest.json',{'builder_commit':commit,'cards':cards,'table':table,'summary':summary})]:
