...
function render(){const item=items[index],a=answers[item.review_id]||{};$('position').textContent=`${index+1} / ${items.length} · ${item.review_id}`;$('decision').textContent=a.decision?`· 当前：${{KEEP:'保留',REMOVE:'去掉',UNCERTAIN:'待定'}[a.decision]}`:'· 未审核';primary.src=item.image;if(item.reference_image)reference.src=item.reference_image;else reference.removeAttribute('src');reference.classList.toggle('show',Boolean(item.reference_image&&referenceShown));viewer.classList.toggle('split',Boolean(item.reference_image&&referenceShown));$('toggleReference').classList.toggle('hidden',!item.reference_image);$('toggleReference').textContent=referenceShown?'R · 隐藏参考图':'R · 显示参考图';note.value=a.note||'';$('jump').value=String(index+1);document.querySelectorAll('[data-decision]').forEach(b=>b.classList.toggle('active',b.dataset.decision===a.decision));$('imageError').textContent='';stats()}
primary.onerror=()=>{$('imageError').textContent='原图加载失败，请停止审核并报告此编号。'};reference.onerror=()=>{if(items[index].reference_image)$('imageError').textContent='附加参考图加载失败，请停止审核并报告此编号。'};
