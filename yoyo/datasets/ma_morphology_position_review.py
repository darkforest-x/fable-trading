...
def build(output: Path):
    ...
    for item in selection['items']:
        ...
        cards.append(f'<article><h2>{title}</h2><div class="views">'+''.join(cells)+f'</div><details><summary>展开同一事件的未来走势（仅供审核）</summary><p>红框是同一原标签；蓝线是原后5根截止，之后额外展示40根。未来走势不进入上面三张训练图。</p><a href="{ident}_future.png" target="_blank"><img src="{ident}_future.png"></a></details></article>')
