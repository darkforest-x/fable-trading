...
(() => {
  ...
  function render() {
    ...
    $('research-overview').innerHTML=`<div class="research-objective"><span class="research-kicker">研究目标</span><h2>${esc(o.objective)}</h2><p>均线密集 → 启动识别 → 因子筛选 → 回测对照 → 前向观察</p></div>
      ...
      <div class="research-flow">${[['01','定义因子','记录公式、窗口和可知时点','factors'],['02','登记实验','明确问题与唯一变化','experiments'],['03','运行回测','冻结成本、时间与原始数据','backtests'],['04','复核证据','核对前后段及随机对照','experiments'],['05','VLM验证','回放图表、参考图与人工判断','vision']].map(([n,t,d,v])=>`<button data-research-nav="${v}"><span>${n}</span><strong>${t}</strong><small>${d}</small><b>↗</b></button>`).join('')}</div>
  ...
  function yoloEvidence(data) {
    const y=data.yolo_evidence;if(!y)return '';
    const qualification=(value)=>value===true?'登记为是（未重新核验）':value===false?'登记为否':'未声明';
    return `<section class="research-factor-section"><h3>YOLO 记录与产物</h3><p class="research-caption">${esc(y.note)}</p>${(y.receipts||[]).length?`<details class="research-json" open><summary>落盘回执状态（与原登记分别保留）</summary>${y.receipts.map(r=>`<p><strong>${esc(r.status)}</strong><br><small class="research-mono">${esc(r.path)}</small></p>`).join('')}</details>`:''}${(y.training_runs||[]).map(r=>`<details class="research-json" open><summary>历史训练 · ${esc(r.recorded_rows)} 条记录 · 最后 epoch ${esc(r.last_epoch??'超出预览')}</summary><p class="research-caption">${esc(r.note)}</p><p class="research-mono">${esc(r.path)}</p><dl class="research-facts">${Object.entries(r.metrics||{}).map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v??'缺失')}</dd>`).join('')}</dl></details>`).join('')}<details class="research-json"><summary>已登记产物（${(y.artifacts||[]).length}）</summary>${(y.artifacts||[]).map(a=>`<article><h4>${esc(a.artifact_id)}</h4><p>${esc(a.artifact_type)} · ${a.local_exists?'本地文件存在':'本地未找到或不可读'}</p><p class="research-mono">${esc(a.source_path||'路径未提供或超出读取范围')}</p><p>训练资格：${qualification(a.training_eligible)} · 生产资格：${qualification(a.production_eligible)}</p><p class="research-mono">登记 SHA：${esc(a.sha256||'未登记')}</p></article>`).join('')||'<p>该实验没有关联的产物登记。</p>'}</details></section>`;
  }
