    ...
    def list_events(self, limit=200, symbol=None, timeframe=None, kind=None, side=None, protocol=None,
        ...
        def decorate(raw):
            ...
            if isinstance(link, dict):
                ...
                compact["covered_ledger"] = {"link_status": link.get("link_status"),
                                            **({"outcome": {k: outcome.get(k) for k in ("status", "exit_reason", "exit_time_ms", "net_r") if k in outcome}} if isinstance(outcome, dict) else {})}
            return compact
        if timing is None:
            with self.connect() as db:
                return [decorate(r) for r in db.execute(sql, values)]
