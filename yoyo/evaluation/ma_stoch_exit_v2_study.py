"""Exploratory second-round exit search with frozen development selection.

Protocol: exp-ma-stoch-exit-optimization-20260915-v2/PROJECT_PLAN.md. Calls the
...
from yoyo.evaluation.ma_stoch_exit_v2 import POLICIES, prepare, simulate, BAR
from yoyo.evaluation.ma_stoch_exit_engine import simulate as simulate_v1
...
EXP=ROOT/'experiments/active/exp-ma-stoch-exit-optimization-20260915-v2'
BUILDERS=['yoyo/evaluation/ma_stoch_exit_v2_study.py','yoyo/evaluation/ma_stoch_exit_v2.py','yoyo/evaluation/ma_stoch_exit_engine.py',
          ...
          'tests/test_ma_stoch_exit_v2.py']
...
def select(arms,minimum=30):
    eligible=[k for k,v in arms.items() if v['n']>=minimum and k not in ('baseline','stop_2','stop_3')]
...
def run(phase):
    ...
    if phase=='recheck':summary['selection_identity']=frozen
    for name in names:
        ...
        if name in ('baseline','stop_2','stop_3'):
            old_v1=simulate_v1(ctx,start,end,name)
            pd.testing.assert_frame_equal(old_v1['trades'],r['trades'])
            np.testing.assert_allclose(old_v1['curve'].equity,r['curve'].equity,atol=1e-10)
            stats['v1_parity']=True
        flags=r['trades'].exit_reason.isin(['stop','stop_gap','close_stop']).to_numpy()
        longest=current=0
        for flag in flags:
            current=current+1 if flag else 0;longest=max(longest,current)
        stats['longest_stop']=longest
        loss_flags=flags & (r['trades'].net_return.to_numpy()<0)
        longest=current=0
        for flag in loss_flags:
            current=current+1 if flag else 0;longest=max(longest,current)
        stats['longest_losing_stop']=longest
        summary['arms'][name]=stats
        print(name,json.dumps({k:stats[k] for k in ('n','mtm_return_pct','mtm_max_drawdown_pct','net_win_rate','longest_loss','longest_stop','net_mean_bp')}),flush=True)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--phase',required=True,choices=['dev','recheck'])
