"""Frozen research runner for visible IMACD startup quality; no live mutations.

Source: exp-imacd-startup-quality-20260908-v1/PROJECT_PLAN.md. Formation
...
END = pd.Timestamp('2026-07-12', tz='UTC')
FOLDS = [('development', '2023-01-01', '2025-01-01'),
         ('validation', '2025-01-01', '2026-01-01'),
         ('audit_pre', '2026-01-01', '2026-05-04'),
         ('holdout_review', '2026-05-04', '2026-07-12')]
POLICIES = ['P00', 'P01', 'P02', 'P03', 'P04']
PERIODS = [15, 60, 240]
SEED = 20260908
SCORES = ['strength_score', 'contraction_score', 'proximity_score', 'separation_score']
...
def policy_masks(f):
    """Five preregistered policies; do not accept any outcome columns."""
    base = f.release_side.ne(0)
    a = base & f.keep_contraction
    b = a & f.keep_proximity
    return {'P00': base, 'P01': a, 'P02': b, 'P03': b & f.keep_separation,
            'P04': base & f.dense}
...
def run():
    ...
    watched = [Path(__file__), ROOT/'yoyo/evaluation/imacd_startup_quality.py',
               ROOT/'yoyo/evaluation/imacd_startup_accounting.py', EXP/'PROJECT_PLAN.md']
    for p in watched:
    ...
    (results/'started.json').write_text(json.dumps(dict(builder_commit=builder,
        ...
        holdout_consumptions={p: 1 for p in POLICIES},
            ...
            for fold_number, (fold, start, end) in enumerate(FOLDS):
                ...
                for i, label in zip(decisions, labels):
                    ...
                    row = dict(**label, event_id=eid, symbol=symbol, minutes=minutes, fold=fold,
                               ...
                               proximity_score=-r.proximity_atr, separation_score=r.separation_delta,
    ...
    for (fold, minutes, policy), eq in pool_equities.items():
        ...
        daily = full.resample('1D').last().dropna()
    ...
    ix = inc.loc[(inc.fold=='validation') & inc.incremental_p.notna()].sort_values('incremental_p').index
    ...
    manifest = dict(builder_commit=builder, completed_at=pd.Timestamp.now(tz='UTC').isoformat(),
                    sources=sources, policies=POLICIES, folds=FOLDS, holdout_consumptions={p:1 for p in POLICIES},
    ...
    print(summary.loc[summary.fold=='holdout_review', ['minutes','policy','n','mean_net_bp','excess_bp','portfolio_net_pct','portfolio_mdd_pct','tail_profit_pct']].to_string(index=False), flush=True)
