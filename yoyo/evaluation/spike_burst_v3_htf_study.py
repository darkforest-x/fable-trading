"""Preregistered fifth hypothesis: completed4H IMACD relative-momentum veto.

E only refuses a V3 parent when its most recent fully closed 4H bar has 341
complete-bar warmup and md<sb, using original34/9. Unknown background leaves V3
eligible, is tagged separately and cannot become claimed HTF approval/noise
reduction. Parent metadata is frozen at its actual1H decision; original1H child
conditions, accepted-only cooldown, fixed risk/next-open execution and20bp stay.
Global preparation precedes economic reads. A-D p values remain frozen in five-
hypothesis Holm. Prior13 explorations and reused history forbid blindOOS claims.
"""
...
from yoyo.evaluation import spike_burst_v3_htf_gate as htf
...
ROOT = old.ROOT
EXPERIMENT = ROOT / "experiments/active/exp-spike-v3-htf-gate-20260910-v1"
OUTPUT = EXPERIMENT / "results"
CACHE_SOURCE = ROOT / "experiments/active/exp-spike-v3-formation-gate-20260910-v1/results"
...
PREPARED_SHA = "84c60082d9df02deb8f2dbeb5f9043559e3d963abb682490c7160b334158fbd4"
VALIDATION_SHA = "e9847380c17d74f24cc1db8b065d387d8e621286dd46069376e36162d5c005f9"
ARMS = ("v3", "htf_gate")
CONFIG = dict(
    schema="spike-v3-closed-4h-veto-v1", arms=ARMS,
    new_hypothesis="E: only veto a parent when completed 4H IMACD is ready341 and md<sb",
    unknown_policy="keep V3 eligibility; explicitly unknown, never count as HTF approval",
    aggregation="complete UTC4H from four original 1H bars; 4H close<=1H decision; exact close allowed",
    interventions="one closed4H relative-momentum veto; no A-D gates or reference suppression",
    primary_economics="parent early only; confirmations retain original1H conditions and are upgrades",
    structural_hypothesis_number=5, holm_family="frozen A, B, C, D and new E full-period p values",
    ...
    execution="unchanged actual next-open; original risk/exits/cost; no detection occupancy",
    cost_bp=20, production_eligible=False,
)
STATE_COLUMNS = (
    "candidate_edge", "cooldown_blocked", "htf_epoch", "htf_close", "htf_age_hours", "htf_count", "htf_ready",
    "htf_md", "htf_sb", "htf_unknown", "htf_opposed", "htf_gatepass", "htf_blocked",
)


def source_pins():
    paths = [Path(__file__), EXPERIMENT / "PROJECT_PLAN.md", EXPERIMENT / "qa/preflight_review.json",
             ROOT / "tests/test_spike_burst_v3_htf_study.py",
             ROOT / "tests/test_spike_burst_v3_htf_gate.py"]
    paths += [ROOT / "yoyo/evaluation" / name for name in (
        "spike_burst_v3_htf_gate.py", "spike_burst_early_warning.py",
...
def frozen_inputs():
    """Authenticate historical D and its A/B/C/V3 sources without using returns in detection."""
    refs, receipts = {}, []
    ...
    if validation["prepared_sha"] != PREPARED_SHA:
        raise ValueError("Historical D outcomes belong to a different schedule")
    ...
    if refs.get(str(AB_SOURCE / "validation_manifest.json")) != AB_VALIDATION_SHA:
        raise ValueError("Historical A/B receipt must remain in the fixed five-hypothesis family")
...
def prepare():
    ...
    for job_number, job in enumerate(jobs):
        ...
        machines = {"v3": v3.detect(frame),
                    "htf_gate": htf.detect(frame)}
        required = set(STATE_COLUMNS)
        if not required.issubset(machines["htf_gate"].columns):
            raise ValueError("E detector must expose closed4H metadata and unknown separately")
        ...
        for arm, state in machines.items():
            ...
            for stage in ("early", "confirmed"):
                ...
                event_positions = detected.event_i.to_numpy(int)
                detected["htf_unknown_at_event"] = machines["htf_gate"].htf_unknown.to_numpy()[event_positions]
                detected["htf_opposed_at_event"] = machines["htf_gate"].htf_opposed.to_numpy()[event_positions]
                ...
                for i in indices:
                    ...
                    signals.append(dict(
                        ...
                        htf_parent_close=machines["htf_gate"].htf_close.iloc[parent_i],
                        htf_parent_count=int(machines["htf_gate"].htf_count.iloc[parent_i]),
                        htf_parent_age_hours=float(machines["htf_gate"].htf_age_hours.iloc[parent_i]),
                        htf_parent_ready=bool(machines["htf_gate"].htf_ready.iloc[parent_i]),
                        htf_parent_unknown=bool(machines["htf_gate"].htf_unknown.iloc[parent_i]),
                        htf_parent_opposed=bool(machines["htf_gate"].htf_opposed.iloc[parent_i]),
                        htf_parent_md=float(machines["htf_gate"].htf_md.iloc[parent_i]),
                        htf_parent_sb=float(machines["htf_gate"].htf_sb.iloc[parent_i]),
        ...
        e, b = machines["htf_gate"], machines["v3"]
        exposures.append(pd.DataFrame(dict(decision_time=clocks[window],
            eligible=frame.ready.to_numpy()[window].astype(int),
            htf_unknown_eligible=(frame.ready & e.htf_unknown).to_numpy()[window].astype(int),
            htf_ready_eligible=(frame.ready & e.htf_ready).to_numpy()[window].astype(int),
            v3_early_unknown=(b.early & e.htf_unknown).to_numpy()[window].astype(int),
            v3_early_opposed=(b.early & e.htf_opposed).to_numpy()[window].astype(int),
            e_early_unknown=(e.early & e.htf_unknown).to_numpy()[window].astype(int),
            e_early_known=(e.early & e.htf_ready).to_numpy()[window].astype(int),
            e_blocked_by_htf=e.htf_blocked.to_numpy()[window].astype(int))))
...
def structural_holm(ab_summary, c_summary, d_summary, current_summary):
    """Keep all five fixed structural hypotheses; history is not rescored."""
    rows = []
    for arm, table, origin in (("reference", ab_summary, "frozen_A"),
                               ("near_box", ab_summary, "frozen_B"),
                               ("confirmation_gate", c_summary, "frozen_C"),
                               ("formation_gate", d_summary, "frozen_D"),
                               ("htf_gate", current_summary, "fifth_exploratory_hypothesis_E")):
    ...
    family["holm_p_five"] = v3.holm(family.correction_input_p.to_numpy())
    family["interpretation"] = "five sequential exploratory hypotheses plus prior 13 gate explorations; not blind OOS"
    return family
...
def retention_summary(labels, detections, signals, exposure):
    ...
    for stage in ("early", "confirmed"):
        ...
        for arm in ARMS:
            ...
            if merged[["hit_1", "hit_6", "baseline_hit_1", "htf_unknown_at_event", "htf_opposed_at_event"]].isna().any().any():
            ...
            for period, (start, end) in old.period_windows().items():
                ...
                rows.append(dict(arm=arm, stage=stage, period=period, signals=len(s),
                    ...
                    htf_unknown_positive_events=int(p.htf_unknown_at_event.sum()),
                    htf_opposed_positive_events=int(p.htf_opposed_at_event.sum()),
                    htf_unknown_signals=int(s.htf_parent_unknown.sum()),
                    htf_known_signals=int(s.htf_parent_ready.sum()),
...
def background_summary(exposure):
    """Coverage and known-opposition vetoes; unknown is never counted as approval."""
    rows = []
    for period, (start, end) in old.period_windows().items():
        x = exposure[exposure.decision_time.ge(start) & exposure.decision_time.lt(end)]
        sums = x.drop(columns=["decision_time"]).sum()
        if sums.htf_unknown_eligible + sums.htf_ready_eligible != sums.eligible:
            raise ValueError("Every eligible hour needs a separate known/unknown HTF status")
        rows.append(dict(period=period, eligible_1h_hours=int(sums.eligible),
            htf_unknown_hours=int(sums.htf_unknown_eligible), htf_ready_hours=int(sums.htf_ready_eligible),
            unknown_fraction=float(sums.htf_unknown_eligible / sums.eligible) if sums.eligible else np.nan,
            v3_early_unknown=int(sums.v3_early_unknown), v3_early_opposed=int(sums.v3_early_opposed),
            e_early_unknown=int(sums.e_early_unknown), e_early_known=int(sums.e_early_known),
            e_edges_blocked_by_known_opposition=int(sums.e_blocked_by_htf),
            unknown_policy="Retain V3 eligibility, not HTF approval"))
    return pd.DataFrame(rows)


def assessment(retention, trades):
    r = retention[retention.arm.eq("htf_gate") & retention.stage.eq("early") & retention.period.eq("full")].iloc[0]
    t = trades[trades.arm.eq("htf_gate") & trades.period.eq("full")].iloc[0]
    return dict(label_drop_pass=bool(r.label_drop >= .5 and r.distinct_labels <= 6021), large_recall_pass=bool(r.large_recall_1 >= .8),
                ...
                economic_pass=bool(t.mean_excess_bp > 0 and t.holm_p_five < .01),
                thresholds=CONFIG["acceptance"], account_nav_not_computed=True,
                live_deployed=False, accepted_for_deployment=False,
                interpretation="Exploratory fifth hypothesis; unknown background retained, fixed fresh-signal denominator")
    ...
    for period in ("full", "first31", "last30", "case_night"):
        for arm in ("v1",) + ARMS:
            if arm == "v1":
                ...
                source = "current V3/E common controls; parent-only economics"
            rows.append(dict(arm=arm, period=period, parent_signals=r.signals, distinct_labels=labels,
                ...
                mean_excess_bp=t.mean_excess_bp, control_comparable_to_new_E=arm != "v1", source=source,
...
def evaluate(workers=3):
    ...
    d_summary = pd.read_csv(CACHE_SOURCE / "trade_summary.csv", float_precision="round_trip")
    family = structural_holm(ab_summary, c_summary, d_summary, summary)
    mask = summary.arm.eq("htf_gate") & summary.period.eq("full")
    summary.loc[mask, "holm_p_five"] = family[family.arm.eq("htf_gate")].holm_p_five.iloc[0]
    ...
    products = {"trade_events.csv.gz": actual, "trade_controls.csv.gz": controls,
                ...
                "retention_summary.csv": retention, "background_summary.csv": background_summary(tables["exposure"]),
    ...
    old.write_json(OUTPUT / "validation_manifest.json", dict(
        ...
        prior_AB_validation_sha=AB_VALIDATION_SHA, prior_C_validation_sha=C_VALIDATION_SHA, prior_D_validation_sha=VALIDATION_SHA,
        artifacts=[old.artifact(OUTPUT / name) for name in list(products) + ["assessment.json"]],
        limitations=["Fifth sequential exploratory hypothesis on previously studied history, not blind OOS",
                     "A/B/C/D p values remain frozen; all five structural hypotheses receive Holm correction",
                     "Unknown4H background is retained and reported separately, never called approval or noise reduction",
