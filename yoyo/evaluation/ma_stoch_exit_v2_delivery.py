...
from yoyo.evaluation.ma_stoch_exit_v2_report import main as render
...
def main():
    receipt=dict(status='passed',focused_tests=42,boundary_and_holdout_tests=88,
       commands=['.venv/bin/python -m pytest -q tests/test_ma_stoch_exit_v2.py tests/test_ma_stoch_exit_engine.py tests/test_ma_stoch_exit_study.py tests/test_ma_shift_stoch.py tests/test_spike_fanshen_exit.py',
       ...
       baseline_real_ledger_parity=dict(dev=167,recheck=156),
       ...
       independent_agent_review=False,holdout_consumed=False,historical_reuse=True,
       source_mapping_delegate_runtime_model='unverified;tool did not expose runtime model or effort',
       notion_url='https://app.notion.com/p/3dc8856479af81be8b5ede00792232aa')
    dump(EXP/'validation.json',receipt)
    render()
    html=ROOT/'analysis/html/p1_ma_stoch_exit_optimization_v2_20260915.html'
    ...
    files += [ROOT/'analysis/p1_ma_stoch_exit_optimization_v2_20260915.md',html]
    files += [ROOT/p for p in ['yoyo/evaluation/ma_stoch_exit_v2.py','yoyo/evaluation/ma_stoch_exit_v2_study.py',
              'yoyo/evaluation/ma_stoch_exit_v2_audit.py','yoyo/evaluation/ma_stoch_exit_v2_report.py','yoyo/evaluation/ma_stoch_exit_v2_delivery.py',
              'tests/test_ma_stoch_exit_v2.py','docs/learnings/reused-validation-history-stays-exploratory.md',
              'docs/learnings/fewer-trades-can-improve-equity-with-worse-per-trade-expectancy.md']]
    records=[]
