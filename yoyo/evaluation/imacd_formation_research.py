"""Frozen V2 formation-memory comparison; no live mutations.

Source: exp-imacd-formation-memory-20260908-v2/PROJECT_PLAN.md. Formation
...
EXP = ROOT / 'experiments/active/exp-imacd-formation-memory-20260908-v2'
DATA = ROOT / 'data/imacd_formation_memory_20260908_v2'
...
PERIODS = [15, 60, 240]
SEED = 20260908
SCORES = ['strength_score', 'contraction_score', 'proximity_score', 'memory_score']
...
def policy_masks(f):
    """Outcome-blind sequential gates; Q02 is the sole candidate."""
    base = f.release_side.ne(0)
    return {'P00': base, 'P02': base & f.keep_contraction & f.keep_proximity,
            'Q01': base & f.keep_memory,
            'Q02': base & f.keep_memory & f.keep_proximity}
...
def run():
    ...
    ix = inc.loc[(inc.fold=='validation') & inc.policy.eq('Q02') & inc.incremental_p.notna()].sort_values('incremental_p').index
