...
def replay_references(ohlcv: pd.DataFrame, tick: float, *, enable_plus: bool) -> pd.DataFrame:
    ...
    for i, row in enumerate(f.itertuples(index=False)):
        ...
        if bool(row.ready):
            ...
            lead_up = can_lead and (not ended) and price_burst(1, o,h,l,c,row.md,row.sb,row.middle,previous_middle,quiet_high,quiet_low,row.ropeHigh,row.ropeLow,row.rv,row.expansion)
            lead_down = can_lead and (not ended) and price_burst(-1, o,h,l,c,row.md,row.sb,row.middle,previous_middle,quiet_high,quiet_low,row.ropeHigh,row.ropeLow,row.rv,row.expansion)
            ...
            if leading and (trend == 0 or leading != trend):
                ...
                if quiet >= MIN_QUIET:
                    ...
                    if dense and side and not ended and (trend == 0 or side != trend):
        ...
        rows.append({"bar_open": f.index[i], "signal": event, "raw_signal": raw_signal, "side": event_side, "signal_reason": event_reason,
                     ...
                     "reference_exit_price": exit_price, "trend_side": trend, "signal_i": i if event else math.nan,
...
def simulate_next_open(ohlcv: pd.DataFrame, refs: pd.DataFrame, *, tick: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Execute reference events at next observed open without changing their stop clock."""
    if not refs.index.equals(ohlcv.index):
        raise ValueError("reference and OHLC clocks must match")
    trades: list[dict] = []; fills: list[dict] = []
    pos: dict | None = None; pending_entry: dict | None = None; pending_reverse = False; next_id = 0
    for i, stamp in enumerate(ohlcv.index):
        r, bar = refs.iloc[i], ohlcv.iloc[i]
        o,h,l,c = (float(bar[k]) for k in ("open","high","low","close"))
        # A protection known at the preceding close is always first.
        if pos is not None:
            active = float(r.active_reference_protection)
            if math.isfinite(active) and ((l <= active) if pos["side"] == 1 else (h >= active)):
                px = min(o, active) if pos["side"] == 1 else max(o, active)
                why = "protective_stop_gap" if ((o <= active) if pos["side"] == 1 else (o >= active)) else "protective_stop"
                gross = pos["side"] * (px / pos["entry_price"] - 1.0); net = gross - FEE
                trades.append({**pos,"exit_i":i,"exit_time":stamp,"exit_price":px,"exit_reason":why,"gross_return":gross,"net_return":net,"gross_r":gross/pos["actual_risk_frac"],"net_r":net/pos["actual_risk_frac"],"censored":False})
                fills.append({"trade_id":pos["trade_id"],"leg_no":2,"kind":"exit","bar_open":stamp,"price":px,"reason":why,"execution_phase":"intrabar" if why.endswith("stop") else "open","qty_fraction":1.0})
                pos=None; pending_reverse=False
        if pos is not None and pending_reverse:
            px=o; gross=pos["side"]*(px/pos["entry_price"]-1.0); net=gross-FEE
            trades.append({**pos,"exit_i":i,"exit_time":stamp,"exit_price":px,"exit_reason":"opposite_reference_next_open","gross_return":gross,"net_return":net,"gross_r":gross/pos["actual_risk_frac"],"net_r":net/pos["actual_risk_frac"],"censored":False})
            fills.append({"trade_id":pos["trade_id"],"leg_no":2,"kind":"exit","bar_open":stamp,"price":px,"reason":"opposite_reference_next_open","execution_phase":"open","qty_fraction":1.0}); pos=None; pending_reverse=False
        if pending_entry is not None and pos is None:
            sig = pending_entry; stop=float(sig["reference_initial_stop"]); risk=abs(o-stop)
            through_stop = (o <= stop) if int(sig.side) == 1 else (o >= stop)
        ...
        if bool(r.reference_exit) and str(r.reference_exit_reason) == "opposite_reference": pending_reverse=True
        if bool(r.signal) and str(r.signal_reason) == "accepted": pending_entry=r
    if pos is not None:
        last=ohlcv.iloc[-1]; px=float(last.close); gross=pos["side"]*(px/pos["entry_price"]-1.0); net=gross-FEE
        trades.append({**pos,"exit_i":len(ohlcv)-1,"exit_time":ohlcv.index[-1],"exit_price":px,"exit_reason":"boundary_mark","gross_return":gross,"net_return":net,"gross_r":gross/pos["actual_risk_frac"],"net_r":net/pos["actual_risk_frac"],"censored":True})
        fills.append({"trade_id":pos["trade_id"],"leg_no":2,"kind":"censor","bar_open":ohlcv.index[-1],"price":px,"reason":"boundary_mark","execution_phase":"close","qty_fraction":1.0})
    return pd.DataFrame(trades), pd.DataFrame(fills)
