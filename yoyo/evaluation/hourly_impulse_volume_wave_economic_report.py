"""Saved V32 report preparation and canonical portable-artifact packaging.
...
REL = Path("experiments/active/exp-btcusdtp-1h-volume-wave-economics-preholdout-20260907-v32")
HERE = ROOT/REL
REPORT = Path("analysis/p1_btcusdtp_hourly_volume_wave_economics_v32_20260907.md")
TITLE = "Volume Wave Economics · V32"
QUERIES = {
 "groups": """SELECT c.wave_gate_state AS state,COUNT(*) AS events,
 ...
 SUM(c.gross_markout<0) AS wrong_direction, SUM(c.gross_markout=0) AS flat_gross,
 ...
 WHERE c.horizon_hours=4 GROUP BY c.wave_gate_state ORDER BY state""",
 "halves": """SELECT c.fold,COUNT(*) AS original_n,
 SUM(c.wave_gate_state='accepted') AS accepted_n,
 AVG(c.cost_threshold_markout)*10000 AS baseline_bp,
 AVG(CASE WHEN c.wave_gate_state='accepted' THEN c.cost_threshold_markout END)*10000 AS accepted_bp,
 AVG(CASE WHEN c.wave_gate_state='accepted' THEN m.control_mean_cost_threshold_markout END)*10000 AS accepted_controls_bp,
 AVG(c.policy_cost_threshold_markout)*10000 AS policy_bp,
 ...
 FROM mothers WHERE horizon_hours=4 AND wave_gate_state='accepted'
 GROUP BY mother_month ORDER BY mother_month""",
 "by_direction": """SELECT c.direction,c.wave_gate_state AS state,COUNT(*) AS events,
 AVG(c.gross_markout)*10000 AS gross_bp,AVG(c.cost_threshold_markout)*10000 AS net_bp,
 SUM(c.cost_threshold_markout>0) AS positive,
 AVG(m.control_mean_cost_threshold_markout)*10000 AS controls_bp,
 AVG(m.cost_threshold_excess_markout)*10000 AS excess_bp,
 COUNT(m.cost_threshold_excess_markout) AS paired_n
 FROM cases c JOIN mothers m USING(event_id,horizon_hours)
 WHERE c.horizon_hours=4 GROUP BY c.direction,c.wave_gate_state ORDER BY c.direction,state""",
 "loss_ledger": """SELECT c.event_id,c.decision_time,c.fold,c.direction,c.wave_gate_state,
 ...
 CASE WHEN c.gross_markout IS NULL THEN 'unknown_label'
 WHEN c.gross_markout<0 THEN 'wrong_direction' WHEN c.gross_markout=0 THEN 'flat_gross'
 ...
 SUM(wave_gate_state='abstain') AS abstain_n,
 AVG(CASE WHEN wave_gate_state='abstain' THEN 0.002 ELSE 0 END)*10000 AS avoided_cost_bp,
 AVG(CASE WHEN wave_gate_state='abstain' THEN -gross_markout ELSE 0 END)*10000 AS avoided_gross_bp,
 ...
 FROM cases WHERE horizon_hours=4 AND wave_gate_state!='unknown'""",
}
...
def checked():
    ...
    if subprocess.check_output(["git", "show", commit+":"+"yoyo/evaluation/hourly_impulse_volume_wave_economic_report.py"], cwd=ROOT) != Path(__file__).read_bytes():
...
def prepare():
    ...
    expected = dict(accepted=100, abstain=148, unknown=3)
    if {r["state"]: r["events"] for r in data["groups"]} != expected or len(data["monthly"]) != 24:
        raise ValueError("Frozen categories/month coverage drifted")
    for row in data["groups"]:
        if row["wrong_direction"]+row["flat_gross"]+row["cost_erased"]+row["positive"] != row["events"]:
            raise ValueError("Outcome categories fail to partition original events")
    import math
    dec = data["policy_decomposition"][0]
    if not math.isclose(dec["avoided_cost_bp"]+dec["avoided_gross_bp"], dec["delta_bp"], abs_tol=1e-9):
        raise ValueError("Cost decomposition does not sum")
...
def package():
    ...
    if subprocess.check_output(["git", "show", commit+":"+str(REPORT)], cwd=ROOT) != (ROOT/REPORT).read_bytes():
        raise ValueError("Commit narrative before artifact build")
    md = (ROOT/REPORT).read_text()
    sections = [s.strip() for s in re.split(r"(?m)(?=^## )", md.strip())]
    if not md.startswith("# "+TITLE+"\n") or len(sections) < 8:
        raise ValueError("Incomplete reader-facing narrative")
    stamp = pd.Timestamp.now(tz="UTC").isoformat()
    sources = [dict(id="report", label="V32 · 完整指标、方法与风险", path=str(REPORT))]
    for name in ("monthly", "groups"):
        sources.append(dict(id=name, label="V32 · 保存逐母标签的真实描述查询", path=str(REL/"report_data.json"),
            query=dict(sql=QUERIES[name], engine="sqlite", language="sql", tables_used=["cases", "mothers"],
                ...
                metric_definitions={"net_bp":"(direction*(open[E+4h]-open[E])/open[E]-0.002)*10000; group arithmetic mean",
                    "controls_bp":"mean of each original three-control mean; no wave filtering in conditional comparator"})))
    monthly = []
    for row in saved["data"]["monthly"]:
        for series, field in (("Accepted K1", "accepted_bp"), ("Original controls", "controls_bp")):
            monthly.append(dict(row, month=row["month"]+"-01", series=series, net_bp=row[field]))
    ...
    charts = [dict(id="monthly", type="line", title="逐月固定4h成本后均值",
        description="2023–2024 UTC · 100个保留入口与原300控制，月内事件加权；不是净值曲线",
