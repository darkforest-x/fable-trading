"""Frozen two-model study of realized net-R>10 on all original V9 long signals.
...
EXP = Path('experiments/active/exp-spike-10r-model-20260921-v4')
...
def dependencies():
    return [Path(__file__), Path(models.__file__), Path(s.__file__), Path(serial.__file__),
            ...
            Path('tests/evaluation/test_spike_10r_models.py'),
            Path('tests/evaluation/test_spike_10r_model_study.py'),
...
def frozen_config(dataset):
    ...
    if (pin != cfg['dataset_receipt_sha256'] or pin != auth['dataset_receipt_sha256']
            or auth['allowed_models'] != list(MODEL_NAMES) or auth['owner_response'] != '继续'
...
def train_predict(dataset, output):
    ...
    if not np.array_equal(strict_bool(c.loc[closed,'label_gt10']).to_numpy(),c.loc[closed,'net_r'].gt(10).to_numpy()):
        raise ValueError('cached labels differ from strict realized net_r > 10')
    ...
    for date in QUARTERS:
        ...
        history = c.loc[train]
        if len(history) < 100 or history.net_r.gt(10).nunique() != 2:
        ...
        folds.append(dict(quarter=tag,fit_start=q-pd.DateOffset(months=12),fit_end=cutoff,
            ...
            train_closed=len(history),train_gt10=int(history.net_r.gt(10).sum()),
        ...
        print(f'{tag}: trained {len(history)}, positives {history.net_r.gt(10).sum()}, predicted {target.sum()}',flush=True)
...
def comparison(c, controls, decisions, serial_mode=False):
    ...
    for period,clock in clocks(baseline):
        ...
        for name in ['original_all',*all_policies()]:
            ...
            summary=s.metrics(selected,universe)
            if serial_mode:
                summary.update(serial.serial_retention(selected,universe))
            rows.append(dict(period=period,rule=name,**summary,
                **s.control_statistics(selected,controls,'full'),**s.rate_interval(universe,selected)))
...
def serial_trigger(table):
    ...
    passed=part.rule.isin(model_policies()) & part.precision.gt(base) & part.gt10.ge(10) & part.recall.ge(.1)
...
def evaluate(dataset,prediction,output):
    ...
    for period,clock in clocks(c):
        for name in (*MODEL_NAMES,'risk'):
            ...
            y=c.iloc[indices].net_r.gt(10).to_numpy(int)
            p=score[indices]
            ...
            row=dict(period=period,score=name,scored=len(y),base_rate=float(y.mean()) if len(y) else np.nan,
                auc_gt10=s.auc_score(y,p),pr_auc=average_precision_score(y,p) if y.sum() else np.nan,
                **s.metrics(top,c.loc[clock]),**s.control_statistics(top,controls,'full'))
    ...
    for name in ['original_all',*all_policies()]:
        ...
        for column in ('asset','timeframe_min','venue','month'):
            for key,g in part.groupby(column):
                groups.append(dict(rule=name,dimension=column,value=str(key),closed=len(g),gt10=int(g.net_r.gt(10).sum()),
                    precision=float(g.net_r.gt(10).mean()),mean_net_bp=float(g.net_return.mean()*1e4)))
    output.mkdir(parents=True)
