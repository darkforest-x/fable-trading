"""Causal fixed-model scoring for the approved offline Spike final-net-R>10 study.
...
@dataclass(frozen=True)
class FittedSpike10RModel:
    """A fixed classifier with only train-derived preprocessing state."""
...
def transform_features(preprocessing: Preprocessing, rows: pd.DataFrame) -> np.ndarray:
    ...
    if preprocessing.feature_names != MODEL_FEATURES:
        raise ValueError("preprocessing feature contract differs from Spike 10R contract")
...
def _labels(train: pd.DataFrame) -> np.ndarray:
    ...
    if pd.api.types.is_bool_dtype(labels):
        ...
        if numeric.isna().any() or not numeric.isin((0, 1)).all():
            raise ValueError("label_gt10 must be boolean or binary 0/1")
        result = numeric.to_numpy(dtype=int)
    if np.unique(result).size != 2:
        raise ValueError("training rows must contain both label_gt10 classes")
    return result


def fit_model(name: str, train: pd.DataFrame) -> FittedSpike10RModel:
    ...
    return FittedSpike10RModel(name=name, preprocessing=preprocessing, estimator=estimator)


def predict_model(fitted: FittedSpike10RModel, rows: pd.DataFrame) -> np.ndarray:
...
def save_model(fitted: FittedSpike10RModel, directory: Path | str) -> dict[str, Path]:
...
def load_model(directory: Path | str) -> FittedSpike10RModel:
    ...
    if name not in MODEL_NAMES:
        raise ValueError("saved model has an unknown model name")
