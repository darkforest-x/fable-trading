"""Source-first V31 support study: prior-K1 direction-volume improvement.

Numeric wave defaults20/5 are attributed to ChartPrime(MPL2.0).
This derived gate is not its native divergence signal or true order flow.

V25 lineage/identity/phase helpers are reused with pinned source hashes.
Read only original identities, frozen V24 sampling metadata and V20 saved OHLCV.
No outcome files, raw prices, new samples,2025+ prices or parameter search.
Timestamp-only preflight precedes saved OHLCV materialization, and input/source
...
from yoyo.evaluation.hourly_impulse_volume_wave_support import OHLCV, build_support

ROOT = Path(__file__).resolve().parents[2]
EXPERIMENT_ID = "exp-btcusdtp-1h-volume-wave-support-preholdout-20260907-v31"
EXPERIMENT = Path("experiments/active") / EXPERIMENT_ID
PINE = "experiments/active/exp-chartprime-public-confluence-audit-20260906-v1/sources/lfaZVLub.pine"
PINS = {PINE: "e92978a8e5fa0873ba25e60c37d0d4b5bea6b33533bbc28cbcb2ad39d2a9e2fe",
...
SOURCES = ["yoyo/evaluation/hourly_impulse_volume_wave_support.py",
           "yoyo/evaluation/hourly_impulse_volume_wave_research.py",
           "tests/test_hourly_impulse_volume_wave_support.py",
           "tests/test_hourly_impulse_volume_wave_research.py",
           "yoyo/evaluation/hourly_impulse_volume_wave_audit.py",
           "tests/test_hourly_impulse_volume_wave_audit.py", *PINS, *parent.FEATURES,
...
def frozen_config():
    ...
    return {**base, "experiment_id": EXPERIMENT_ID,
            "gate": "direction_times_preK1_wave_change_strictly_positive",
            "source_pins": PINS, "parameters": {"wave_length": 20, "smooth_length": 5,
            "ema_adjust": False, "ema_seed": "first_value_per_hourly_segment",
            "preK1_lags_hours": [1, 2], "strict_threshold": 0,
            "minimum_history_at_older_lag": 100, "warmup_reason": "five_times_source_wave_length",
            "zero_volume_denominator_input": 1, "doji_signed_volume": 0,
            "gap_policy": "reset_segment", "native_source_signal_used": False,
...
def verify_inputs(root):
    ...
    return {**parent.verify_inputs(root), "volume_wave_sources": PINS}
...
def run(root=ROOT):
    ...
    if config != frozen_config():
        raise ValueError("V31 frozen config changed")
    ...
    try:
        ...
        hourly = pd.read_csv(path, usecols=OHLCV)
