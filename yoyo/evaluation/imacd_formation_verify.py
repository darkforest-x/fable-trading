"""Independent saved-ledger checks for the frozen V2 formation comparison.

Source: the V2 PROJECT_PLAN and the written CSV contract of
``imacd_formation_research``. This verifier does not import the runner, replay
...
independently reconstructed here. The three-candidate Holm arithmetic is
...
def verify_saved(data_dir: str | Path, results_dir: str | Path) -> dict:
    """Check the saved V2 files once each; never access raw price sources."""
    ...
    events = _read(data_dir / "events.csv.gz", label_columns + [*POLICIES, "month", "near_zero_bars",
        ...
        "formation_memory_background_width", "control_n", "control_mean_net_bp", "excess_bp"])
    ...
    position = np.isfinite(events.proximity_atr) & events.proximity_atr.le(1)
    memory = finite & recent.le(background) & np.isfinite(events.formation_memory_ratio) & events.formation_memory_ratio.le(1)
    old = np.isfinite(events.contraction_ratio) & events.contraction_ratio.le(1) & position
    checks["p00_is_all_original_events"] = bool(events.P00.eq(True).all())
    checks["p02_preserves_old_contraction_and_position"] = bool(events.P02.eq(old).all())
    checks["q01_is_memory_only"] = bool(events.Q01.eq(memory).all())
    checks["q02_adds_unchanged_position_only"] = bool(events.Q02.eq(memory & position).all())
    ...
    candidate = summary.loc[summary.fold.eq("validation") & summary.policy.eq("Q02")].sort_values("incremental_p")
    holm_ok = len(candidate) == 3 and set(candidate.minutes) == set(PERIODS)
    if holm_ok:
        ps = candidate.incremental_p.to_numpy(float)
        expected_holm = np.maximum.accumulate(np.minimum(1, ps * np.array([3, 2, 1])))
        holm_ok = bool(np.isfinite(ps).all() and ((ps >= 0) & (ps <= 1)).all()
                       and _near(candidate.incremental_p_holm_validation, expected_holm))
    other = summary.loc[~(summary.fold.eq("validation") & summary.policy.eq("Q02"))]
    checks["candidate_holm_is_exactly_three_validation_periods"] = bool(holm_ok and other.incremental_p_holm_validation.isna().all())
