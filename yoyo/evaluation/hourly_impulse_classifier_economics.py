"""Pure exploratory V30 economics of the frozen V29 classifier gate.
...
their original three-control aggregates, and V29 own-clock classifier context.
The labels use open[E] through open[E+H]; these are diagnostic outcomes, NEVER
entry features. Context signal_time+1h and classifier_available_at must equal
each request's OWN E. Original 251 mothers / 744 controls and every horizon
...
controls accepted by their classifier. A separate policy comparison applies
...
CONTEXT_COLUMNS = (
    ...
    "mother_id", "control_slot", "mother_month", "matched_support", GATE,
    "classifier_available_at", "classifier_direction", "classifier_known", "classifier_reason",
)
LABEL_COLUMNS = (*base._CASE_COLUMNS, "control_slot", "endpoint_time")
COUNTS = {"case": {"accepted": 78, "abstain": 170, "unknown": 3},
          "control": {"accepted": 136, "abstain": 608, "unknown": 0}}


def _contexts(frame, kind):
    """Validate context before any return calculation; preserve frozen statuses."""
    ...
    for row in rows:
        ...
        signal, decision, available = (_time(row[col]) for col in
                                       ("signal_time", "decision_time", "classifier_available_at"))
        if decision != signal + pd.Timedelta(hours=1) or available != decision:
            raise ValueError("Classifier must be available at own signal+1h decision")
        ...
        matched, known = _boolean(row["matched_support"]), _boolean(row["classifier_known"])
        state, reason = row[GATE], row["classifier_reason"]
        if not isinstance(reason, str) or not reason or (reason == "known") != known:
            raise ValueError("Classifier known/reason disagree")
        if known:
            classifier_direction = _integer(row["classifier_direction"])
            if classifier_direction not in (-1, 0, 1):
                raise ValueError("Invalid known classifier direction")
            expected = "accepted" if classifier_direction == direction else "abstain"
        else:
            if not pd.isna(row["classifier_direction"]):
                raise ValueError("Unknown classifier direction must remain missing")
            classifier_direction, expected = np.nan, "unknown"
        if state != expected:
            raise ValueError("Classifier direction/known/gate disagree")
        if kind == "case":
        ...
        row.update(signal_time=signal, decision_time=decision, classifier_available_at=available,
                   direction=direction, control_slot=slot, matched_support=matched,
                   classifier_known=known, classifier_direction=classifier_direction)
        lookup[event] = row
    ...
    if actual != COUNTS[kind] or len(rows) != sum(COUNTS[kind].values()):
        raise ValueError("Frozen %s classifier population changed" % kind)
    return lookup
...
def analyze(case_labels, control_labels, paired_labels, case_context, control_context):
    ...
    if not all(row["matched_support"] for row in case_ctx.values() if row[GATE] == "accepted"):
        raise ValueError("All 78 frozen accepted mothers require their original control triples")
    ...
    for row, pair in zip(cases.to_dict("records"), pairs.to_dict("records")):
        ...
        pair.update({col: row[col] for col in (GATE, "signal_time", "classifier_available_at", "classifier_direction")})
    ...
    for name in PRIMARY_SERIES:
        # Conditional groups use their actual 78 mothers, not 173 artificial missing rows.
