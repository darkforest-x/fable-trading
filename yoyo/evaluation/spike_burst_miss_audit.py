...
def audit_trace(frame, state):
    ...
    for i, (_, row) in enumerate(frame.iterrows()):
        ...
        item = dict(open_time=t, open_time_bjt=t.tz_convert("Asia/Shanghai"),
            ...
            pending_age=age if during else NAN, retained_release_age=age,
            pending_reference_active=bool(during), pending_age_pass=window_age, pending_md_pass=window_direction,
...
def build(output=OUTPUT, prior_results=OLD):
    ...
    result = dict(schema="spike-burst-owner-miss-audit-v1", generated_at=pd.Timestamp.now(tz="UTC"),
        ...
        explanation_revision=dict(version=2,
            change="Inactive pending_age is null; retained_release_age preserves the historical episode age. No V1 state or parameter change.",
            initial_receipt_path=str(output / "revisions/initial/owner_missed_launches_audit.json"),
            initial_receipt_sha256=sha(output / "revisions/initial/owner_missed_launches_audit.json")
            if (output / "revisions/initial/owner_missed_launches_audit.json").exists() else None),
