"""Receipt-bound High-R Entry V2.1 replay on the complete frozen V9 universe.

Source: V9 full_v1, unchanged exits and initial-risk/cost contract. The
only treatment is the long entry breakout gate implemented in spike_high_r_entry_v21.
...
EXP = Path("experiments/active/exp-spike-high-r-entry-20260921-v21")
SOURCE = Path("experiments/active/exp-spike-v9-full-backtest-20260915-v1")
STATS = SOURCE / "statistics/full_v1"
DEPENDENCIES = (
    Path(__file__), Path("yoyo/evaluation/spike_high_r_entry_v21.py"),
    Path("yoyo/evaluation/spike_high_r_risk_report.py"),
    Path("tests/evaluation/test_spike_high_r_entry_v21.py"),
    Path("tests/evaluation/test_spike_high_r_risk_study.py"),
    ...
    Path("yoyo/evaluation/spike_high_r_risk_calibration.py"),
    EXP / "calibration_v1/thresholds.csv", EXP / "calibration_v1/receipt.json",
...
def validate_contract(cfg):
    ...
    from yoyo.evaluation.spike_high_r_entry_v21 import QUANTILE, LOOKBACK_MONTHS, MIN_HISTORY
    if (cfg["quantile"], cfg["lookback_months"], cfg["min_history"]) != (QUANTILE, LOOKBACK_MONTHS, MIN_HISTORY) or (QUANTILE, LOOKBACK_MONTHS, MIN_HISTORY) != (.1,3,100):
        raise ValueError("risk entry contract differs from frozen plan")
...
def one(args):
    from yoyo.evaluation import spike_high_r_entry_v21 as engine
    ...
    try:
        ...
        thresholds = pd.read_csv(EXP / "calibration_v1/thresholds.csv")
        filtered, gate = engine.prepare_entry_v21(prepared, decisions, thresholds)
        treatment, tf, events = parent.replay_serial(context, arm="v8", enable_be=False, prepared=filtered)
        baseline, treatment = tag(baseline, "baseline"), tag(treatment, "high_r_entry_v21")
        oldc = pd.read_csv(source_dir / "controls.csv.gz")
        oldc = tag(oldc.loc[oldc.arm.eq("v9") & oldc.side.eq(1)], "baseline")
        newc = tag(random_controls(prepared, treatment.loc[treatment.side.eq(1)]), "high_r_entry_v21")
        for control in (oldc, newc):
            enrich_control_bins(control, prepared)
        gate = gate.merge(decisions[["signal_i", "side", "volume_ratio"]], on=["signal_i", "side"], how="left", validate="one_to_one")
        gate = tag(gate, "high_r_entry_v21")
        ...
        tables = {"trades": pd.concat([baseline, treatment], ignore_index=True),
                  ...
                  "fills": pd.concat([tag(bf, "baseline"), tag(tf, "high_r_entry_v21")], ignore_index=True),
        ...
        summary = []
        for arm, frame in (("baseline", baseline), ("high_r_entry_v21", treatment)):
...
def run(output, workers=3, limit=None):
    ...
    calibration = json.loads((EXP / "calibration_v1/receipt.json").read_text())
    if calibration["source_statistics_receipt_sha256"] != source_sha or calibration["thresholds_sha256"] != digest(EXP / "calibration_v1/thresholds.csv"):
        raise ValueError("calibration source/file identity differs")
    if calibration["builder_sha256"] != digest(Path("yoyo/evaluation/spike_high_r_risk_calibration.py")):
        raise ValueError("calibration builder identity differs")
    for name, expected_sha in calibration["files"].items():
        if digest(EXP / "calibration_v1" / name) != expected_sha:
            raise ValueError("calibration evidence identity differs")
