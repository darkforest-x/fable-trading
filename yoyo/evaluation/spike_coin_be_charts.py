...
function list(){let f=$('asset').value,rs=RECORDS.filter(r=>!f||r.asset===f);$('list').innerHTML=rs.map(r=>'<button class="'+(active&&r.case_id===active.case_id?'active':'')+'" onclick="pick(\''+r.case_id+'\')">'+r.case_id+' · '+r.asset+' '+r.timeframe_min+'m<br><small>'+r.case_kind+'</small></button>').join('')}function pick(id){active=RECORDS.find(r=>r.case_id===id);list();render()}[...new Set(RECORDS.map(x=>x.asset))].sort().forEach(x=>$('asset').insertAdjacentHTML('beforeend','<option>'+x+'</option>'));$('asset').onchange=list;pick(RECORDS[0].case_id);</script></html>'''
