...
const RECORDS=__DATA__;const policyNames={baseline:'原退出',be1_price:'1R推入场价',be1_cost:'1R含费保本'},cohortNames={v1_common_long:'V1 · 统一退出',v6_both:'V6',v7_both:'V7'},caseNames={be_reduced_loss:'保本减少亏损',be_failure:'亏损案例',both_ge_10r:'两种退出均兑现10R以上',be_early_trend_exit:'保本提前结束趋势',fallback_illustration:'补充复盘'};function policyName(v){return policyNames[v]||v}function reasonName(v){return ({initial_stop:'初始止损',trailing_stop:'保护止损',raw_reverse_next_open:'反向退出'})[v]||v||'—'}function displayPrice(v){return v==null?'—':Number(v).toFixed(precision(active.tick))}let active,charts=[],syncing=false;const fmt=new Intl.DateTimeFormat('zh-CN',{timeZone:'Asia/Shanghai',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}),$=id=>document.getElementById(id),time=ms=>ms==null?'—':new Intl.DateTimeFormat('zh-CN',{timeZone:'Asia/Shanghai',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(ms))+' 北京';
...
function list(){let f=$('asset').value,rs=RECORDS.filter(r=>!f||r.asset===f);$('list').innerHTML=rs.map(r=>'<button class="'+(active&&r.case_id===active.case_id?'active':'')+'" data-case="'+r.case_id+'">'+r.case_id+' · '+r.asset+' '+r.timeframe_min+'m<br><small>'+(caseNames[r.case_kind]||r.case_kind)+'</small></button>').join('');document.querySelectorAll('#list [data-case]').forEach(node=>node.addEventListener('click',()=>pick(node.dataset.case)))}function pick(id){active=RECORDS.find(r=>r.case_id===id);list();render()}[...new Set(RECORDS.map(x=>x.asset))].sort().forEach(x=>$('asset').insertAdjacentHTML('beforeend','<option>'+x+'</option>'));$('asset').onchange=list;pick(RECORDS[0].case_id);</script></html>'''
    ...
    template = template.replace(
        "['初始SL',displayPrice(r.initial_stop)],['图表范围',r.coverage.bars+' 根']",
        "['初始SL',displayPrice(r.initial_stop)],...r.r_levels.map(x=>[x.label,displayPrice(x.price)]),['图表范围',r.coverage.bars+' 根']",
    )
