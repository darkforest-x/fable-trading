"""V16 saved-only partial-realization notebook with a pinned stdlib verifier.

Both arms' six ledgers plus three paired differences are allowlisted. The
verifier API reconstructs accounting and unchanged final paths from saved
...
from yoyo.evaluation.hourly_impulse_dual_partial_report import BASE_POLICY,CANDIDATE_POLICY,EXPERIMENT_ID
...
def build_notebook(summary_sha256,verifier_hashes):
    ...
    if not valid(summary_sha256) or set(verifier_hashes)!=set(VERIFIER_FILES) or not all(valid(v) for v in verifier_hashes.values()):
        raise ValueError("Summary and fixed self-contained V16 verifier SHA256 pins required")
    cells=[
        _cell("markdown","tldr","""
        # V16 · 快速分批兑现的保存账本复核
        ...
        """),
        _cell("markdown","load-title","### 1. 固定来源与唯一兑现开关"),
        _cell("code","load",f"""
        ...
        require(summary["experiment_id"]=={EXPERIMENT_ID!r},"Wrong V16 experiment")
...
def execute_notebook(notebook,repository_root):
    ...
    result["cells"][0]=_cell("markdown","tldr",f"""
        ...
        原15m版完成交易{facts['baseline_events']}笔、净均值{facts['baseline_mean_net_bp']}bp；
        加一次50%分批版完成交易{facts['candidate_events']}笔、净均值{facts['candidate_mean_net_bp']}bp。
