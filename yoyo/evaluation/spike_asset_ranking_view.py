...
<p style="margin-top:16px">点击表头也可排序。全期收益榜是事后描述，不能据此直接剔除同期亏损币。低样本只供查阅；PF 按 R 与按名义价格收益会不同。R 分母过小、费用占 R 过大及少数极端盈利，都可能扭曲排名。</p>
...
for(const id of ['view','scope','dimension','search','minimum','sort','direction'])by(id).addEventListener('input',render);render();
</script></html>'''
