"""Pure, preregistered V24 fixed-clock descriptive and monthly-cluster analysis.
...
_FOLD_COUNTS = {"2023H1": 55, "2023H2": 66, "2024H1": 55, "2024H2": 75}
...
def _prepare(frame, columns, bounds):
    ...
    if len(identities) != EXPECTED_MOTHERS or len(rows) != EXPECTED_MOTHERS * 4:
        raise ValueError("all original 251 mothers and four horizons must be retained")
    ...
    if sum(item[4] for item in identities.values()) != EXPECTED_SUPPORTED:
        raise ValueError("the frozen support denominator is 248, not an outcome-selected subset")
...
def analyze_fixed_clock_statistics(case_labels, paired_labels, fold_bounds):
    """Analyze the frozen 251-mother V24 tables without mutating either input.
    ...
    >=226 known mothers, mean>0, CI lower>0, p<.01 and positive case cost-threshold
