"""Aggregate immutable High-R Entry V2.1 receipts without reading price history.
...
from yoyo.evaluation.spike_high_r_risk_study import completed, digest, dump
...
def rate_contrast(frame):
    ...
    arms = ("baseline", "high_r_entry_v21")
...
def ranking_diagnostics(long, controls):
    ...
    for period, part in periods(baseline):
        ...
        for feature, score in (("negative_reference_risk_fraction", closed.score), ("volume_ratio", closed.volume_ratio)):
...
def evaluate_gate(summary, rates):
    ...
    old, new = rows.loc["baseline"], rows.loc["high_r_entry_v21"]
...
def run(source, output):
    ...
    for dimension, group, table in groups:
        for arm in ("baseline", "high_r_entry_v21"):
    ...
    for period, p in periods(long):
        ...
        for name, values in (("gt10", closed.net_r.gt(10).astype(float)), ("net_r", closed.net_r),
            ...
            totals = totals.reindex(columns=["baseline", "high_r_entry_v21"], fill_value=0)
            delta = totals.high_r_entry_v21-totals.baseline
    ...
    n = long.loc[long.arm.eq("high_r_entry_v21"), fields]
