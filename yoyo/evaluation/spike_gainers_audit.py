...
def summarize(root):
    ...
    for symbol in symbols:
        ...
        sent = sum(e["bark_status"] == "sent" for r in symbol_rows for e in r["today_visible_longs"])
        rows.append({"symbol": symbol, "periods": symbol_rows, "today_visible_long_count": count,
                     "today_raw_bark_sent": sent,
                     "category": "今天有启动且Bark接受" if sent else "今天有启动但早于周期启用" if count else "今天无当前可见启动"})
