"""V16 full native report over a fixed slow-exit/fast-partial comparison.
...
Both arms use native15m SMA40 true-colour exits. Only the candidate enables
one original-position50% realization on a qualified native5m colour edge.
...
CHART_ID="v16_case_difference_distribution"
MARKER="<!-- dual-partial-delta-chart -->"
BASE_POLICY={"id":"15m_native40","management_minutes":15,"ma_kind":"SMA","ma_length":40,
             "exit_mode":"transition_colour","confirmations":1}
CANDIDATE_POLICY={**BASE_POLICY,"id":"15m_native40_dual_partial","fast_partial_fraction":0.5}
...
def validate_summary(summary):
    ...
    if summary.get("experiment_id")!=EXPERIMENT_ID or summary.get("status")!="diagnostic_only_no_candidate_acceptance":
        raise ValueError("Wrong V16 experiment/status")
    expected={"baseline":BASE_POLICY,"candidate":CANDIDATE_POLICY}
    if set(summary.get("arms",{}))!=set(expected):raise ValueError("Exactly two frozen dual-partial arms required")
    for arm,policy in expected.items():
        if json.dumps(summary["arms"][arm].get("policy"),sort_keys=True)!=json.dumps(policy,sort_keys=True):
            raise ValueError("Only native15m true-colour baseline plus fixed0.5 fast-partial switch allowed")
...
def build_artifact(markdown,summary,case_delta,*,markdown_path,summary_path,case_delta_path,generated_at,fixture=False):
    ...
    translations={"<!-- SOURCE: v16_summary -->":"<!-- SOURCE: v8_summary -->",
                  "<!-- SOURCE: v16_case_delta -->":"<!-- SOURCE: v8_case_delta -->"}
    ...
    for line in markdown.splitlines(keepends=True):
        ...
        if outside and line.strip()==MARKER:
            ...
            raise ValueError("Only V16 source/distribution directives allowed")
        lines.append(line)
    if count!=1:raise ValueError("Exactly one dual-partial-delta-chart marker required")
    ...
    names={"v8_summary":"v16_summary","v8_case_delta":"v16_case_delta"}
    for block in artifact["manifest"]["blocks"]:
        if "sourceId" in block:block["sourceId"]=names.get(block["sourceId"],block["sourceId"])
        if block["type"]=="chart":block.update(id="v16_distribution_chart",chartId=CHART_ID)
    for source in artifact["sources"]:
        source["id"]=names.get(source["id"],source["id"])
        source["label"]=source["label"].replace("V8","V16")
        if source["id"]=="research_code":source["path"]="yoyo/evaluation/hourly_impulse_dual_partial_research.py"
        if source["id"]=="presentation_code":source["path"]="yoyo/evaluation/hourly_impulse_dual_partial_report.py"
        if source["id"]=="v16_case_delta":
            source["query"]["description"]=(
                ...
                "hourly_impulse_dual_partial_research.py supplied all251 original direct1h K1 episode pairs. "
                "Both arms retain native15m SMA40 true-colour full/remainder exits. "
                "Candidate alone can realize50% of the original position once on a native5m true-colour edge, "
                "provided latest completed slow15m colour is aligned and executable-open whole-position gross return exceeds0.002. "
                "The registered invariant requires identical final exit time/price/reason, held duration, "
                "MFE/MAE, slow trigger and serial occupancy; saved-ledger verification checks these separately. "
                "This compares a qualified partial-realization switch, not two management timeframes or an entry gate. "
            ...
            source["query"]["metric_definitions"][0]=(
                ...
                "before=full-position native15m transition-colour exit; after=same exit plus one qualified "
                "50% native5m partial realization. Unknown paired outcomes remain NULL; chart bp=difference*10000.")
    ...
    chart.update(id=CHART_ID,dataset=CHART_ID,sourceId="v16_case_delta",
        title="快速分批退出的逐机会收益变化分布",
        question="保持原入场和15分钟趋势退出不变，增加一次50%兑现如何影响全部机会？",
        subtitle=f"2023–2024 · 全部 {effect['total_pairs']} 原始机会，未知 {effect['unknown_pairs']} · 分批版−原15m版，bp · 分箱计数非密度；零类 |Δ|≤1e−8 bp")
    chart["encodings"]["x"]["label"]="净收益变化区间（bp；分批版−原15m版）"
