"""Preregistered third structural hypothesis: confirmation earns suppression.

Only C versus frozen V3 is detected here. A/B remain immutable historical
hypotheses and contribute their already-frozen p values to a three-hypothesis
Holm family. Every parent/upgrade and common random schedule is prepared before
execution outcomes are loaded. Economic rows are accepted parents only, with
unchanged next-open fills and20bp costs. A signal-close reference or committed
suppression covering an event never repairs a missing fresh signal.

The history has already been inspected. This sequential third hypothesis is
exploratory, not a fresh blind out-of-sample test or an optimized deployment.
"""
...
from yoyo.evaluation import spike_burst_v3_confirmation_gate as confirmation
...
ROOT = old.ROOT
EXPERIMENT = ROOT / "experiments/active/exp-spike-v3-confirmation-gate-20260910-v1"
OUTPUT = EXPERIMENT / "results"
FOCUS = ROOT / "experiments/active/exp-spike-v3-focus-20260910-v1/results"
...
ARMS = ("v3", "confirmation_gate")
CONFIG = dict(
    schema="spike-v3-confirmation-suppression-v1", arms=ARMS,
    new_hypothesis="C: confirmation acquires suppression; unconfirmed temporary reference expires at age4",
    interventions="one new timing intervention; A/B not rerun or stacked",
    primary_economics="parent early only; confirmations are upgrades",
    structural_hypothesis_number=3, holm_family="frozen A, frozen B, new C full-period p values",
    ...
    execution="unchanged actual next-open; detection reference uses signal-close price",
    cost_bp=20, production_eligible=False,
)
STATE_COLUMNS = (
    "reference_active", "reference_before", "reference_owner_i", "reference_protection",
    "reference_exit", "reference_expired_unconfirmed", "holding_blocked",
    "suppression_active", "suppression_before", "suppression_acquired", "suppression_exit",
)


def source_pins():
    paths = [Path(__file__), EXPERIMENT / "PROJECT_PLAN.md",
             ROOT / "tests/test_spike_burst_v3_confirmation_study.py"]
    paths += [ROOT / "yoyo/evaluation" / name for name in (
        "spike_burst_v3_confirmation_gate.py", "spike_burst_early_warning.py",
...
def frozen_inputs():
    """Authenticate A/B/V3 outcomes without using their returns in detection."""
    refs, receipts = {}, []
    for name, digest in (("prepared_manifest.json", PREPARED_SHA),
                         ("validation_manifest.json", VALIDATION_SHA)):
        path = old.checked(FOCUS / name, digest)
    ...
    if validation["prepared_sha"] != PREPARED_SHA:
        raise ValueError("A/B outcomes belong to a different schedule")
    ...
    for path, digest in prepared["sources"].items():
        ...
        refs[path] = digest
    matching = json.loads((FOCUS / "matching.json").read_text())
    labels = pd.read_csv(FOCUS / "labels.csv.gz", float_precision="round_trip")
...
def prepare():
    ...
    for job_number, job in enumerate(jobs):
        ...
        machines = {"v3": v3.detect(frame),
                    "confirmation_gate": confirmation.detect(frame, float(job["tick"]))}
        required = set(STATE_COLUMNS)
        if not required.issubset(machines["confirmation_gate"].columns):
            raise ValueError("C detector must distinguish reference and committed suppression")
        ...
        for arm, state in machines.items():
            ...
            for key in STATE_COLUMNS:
                if key in state:
                    saved[arm + "_" + key] = state[key].to_numpy()
            reference = state.reference_active if "reference_active" in state else pd.Series(False, index=state.index)
            suppression = state.suppression_active if "suppression_active" in state else pd.Series(False, index=state.index)
            for stage in ("early", "confirmed"):
                ...
                dummy = pd.DataFrame(dict(trend_side=reference.astype(int), burst=state[stage]), index=frame.index)
                matched, detected = old.match_signals(local_labels, indices, frame, dummy)
                # Coverage is context at the anchor, independent of future outcome.
                detected["suppression_already_tracking"] = [
                    bool(int(i) > 0 and suppression.iloc[int(i) - 1] and suppression.iloc[int(i)]
                         and not state[stage].iloc[int(i)]) for i in detected.event_i]
                ...
                for i in indices:
                    ...
                    signals.append(dict(
                        ...
                        reference_active=bool(reference.iloc[i]), suppression_active=bool(suppression.iloc[i]),
                        reference_owner_i=state.reference_owner_i.iloc[i] if "reference_owner_i" in state else np.nan,
    ...
    shutil.copyfile(FOCUS / "labels.csv.gz", OUTPUT / "labels.csv.gz")
...
def structural_holm(prior_summary, current_summary):
    """At least A/B/C enter the family; do not reset multiplicity for C."""
    rows = []
    for arm, table, origin in (("reference", prior_summary, "frozen_A"),
                               ("near_box", prior_summary, "frozen_B"),
                               ("confirmation_gate", current_summary, "third_exploratory_hypothesis_C")):
    ...
    family["holm_p_three"] = v3.holm(family.correction_input_p.to_numpy())
    family["interpretation"] = "sequential exploratory hypotheses on previously seen history, not blind OOS"
    return family
...
def retention_summary(labels, detections, signals, exposure):
    ...
    for stage in ("early", "confirmed"):
        ...
        for arm in ARMS:
            ...
            if merged[["hit_1", "hit_6", "baseline_hit_1", "already_tracking", "suppression_already_tracking"]].isna().any().any():
            ...
            for period, (start, end) in old.period_windows().items():
                ...
                rows.append(dict(arm=arm, stage=stage, period=period, signals=len(s),
                    ...
                    reference_tracking_positive_events=int(p.already_tracking.sum()),
                    suppression_tracking_positive_events=int(p.suppression_already_tracking.sum()),
...
def assessment(retention, trades):
    r = retention[retention.arm.eq("confirmation_gate") & retention.stage.eq("early") & retention.period.eq("full")].iloc[0]
    t = trades[trades.arm.eq("confirmation_gate") & trades.period.eq("full")].iloc[0]
    return dict(label_drop_pass=bool(r.label_drop >= .5), large_recall_pass=bool(r.large_recall_1 >= .8),
                baseline_retention_pass=bool(r.retention >= .9),
                economic_pass=bool(t.mean_excess_bp > 0 and t.holm_p_three < .01),
                thresholds=CONFIG["acceptance"], account_nav_not_computed=True,
                deployment_authorized=False, interpretation="Exploratory third hypothesis; coverage never repairs missed fresh signals")
    ...
    for period in ("full", "first31", "last30", "case_night"):
        for arm in ("v1",) + ARMS:
            if arm == "v1":
                ...
                source = "current V3/C common controls; parent-only economics"
            rows.append(dict(arm=arm, period=period, parent_signals=r.signals, distinct_labels=labels,
                ...
                mean_excess_bp=t.mean_excess_bp, control_comparable_to_new_C=arm != "v1", source=source,
...
def initialize_evaluation():
    ...
    cached_jobs = json.loads((FOCUS / "matching.json").read_text())["jobs"]
    ...
    for name in ("trade_events.csv.gz", "trade_controls.csv.gz"):
        for row in pd.read_csv(FOCUS / name, float_precision="round_trip").to_dict("records"):
...
def evaluate(workers=3):
    ...
    prior_summary = pd.read_csv(FOCUS / "trade_summary.csv", float_precision="round_trip")
    family = structural_holm(prior_summary, summary)
    mask = summary.arm.eq("confirmation_gate") & summary.period.eq("full")
    summary.loc[mask, "holm_p_three"] = family[family.arm.eq("confirmation_gate")].holm_p_three.iloc[0]
    ...
    old.write_json(OUTPUT / "validation_manifest.json", dict(
        ...
        prior_AB_validation_sha=VALIDATION_SHA,
        artifacts=[old.artifact(OUTPUT / name) for name in list(products) + ["assessment.json"]],
        limitations=["Third sequential exploratory hypothesis on previously studied history, not blind OOS",
                     "A/B p values remain frozen; at least all three structural hypotheses receive Holm correction",
                     "Reference and confirmed suppression coverage never substitute for timely new parents",
